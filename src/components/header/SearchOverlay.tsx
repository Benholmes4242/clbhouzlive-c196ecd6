...
const CLUBHOUSE_ROUTES = ['/clubhouse', '/watch', '/reels'];
...
export const SearchOverlay: React.FC<SearchOverlayProps> = ({ isOpen, onClose }) => {
  ...
  const isClubhousePage = CLUBHOUSE_ROUTES.some(route => location.pathname.startsWith(route));
  
  // Theme tokens based on route
  const theme = isClubhousePage ? {
    bg: 'rgba(0, 0, 0, 0.85)',
    ...
    bg: 'var(--chrome-light-bg-blur)',
    inputBg: '#FFFFFF',
    inputBorder: 'var(--chrome-light-border)',
    text: 'var(--chrome-light-text)',
    textDim: 'var(--chrome-light-text-dim)',
    icon: 'var(--chrome-light-icon)',
    iconDim: 'var(--chrome-light-icon-dim)',
    border: 'var(--chrome-light-border)',
    hover: 'var(--chrome-light-hover)',
    active: 'var(--chrome-light-active)',
    pillBg: '#FFFFFF',
    pillBorder: 'var(--chrome-light-border)',
  };
      ...
      >
        <div className="max-w-2xl mx-auto">
          ...
          >
            ...
            >
              <X className="h-5 w-5" style={{ color: theme.iconDim }} />
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-2xl mx-auto px-3 py-4">
            ...
            <div className="space-y-2">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="flex items-center gap-4 p-3 rounded-sq-md animate-pulse">
                  <div className="w-10 h-10 rounded-full" style={{ background: theme.hover }} />
                  <div className="flex-1 space-y-2">
                    <div className="w-32 h-4 rounded" style={{ background: theme.hover }} />
                    <div className="w-24 h-3 rounded" style={{ background: theme.hover }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {showEmpty && (
            <div className="py-12 text-center">
              <Search className="h-12 w-12 mx-auto mb-4" style={{ color: theme.iconDim, opacity: 0.5 }} />
              <p className="text-sm" style={{ color: theme.textDim }}>No results found for "{query}"</p>
              <p className="text-xs mt-1" style={{ color: theme.textDim, opacity: 0.7 }}>Try searching by name or course</p>
            </div>
          )}
          ...
          {hasResults && !isLoading && (
            <div className="space-y-6">
              ...
              {peopleResults.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 px-2 mb-2">
                    <User className="h-3.5 w-3.5" style={{ color: theme.iconDim }} />
                    <span className="text-xs font-medium uppercase tracking-wide" style={{ color: theme.textDim }}>People</span>
                  </div>
              ...
              {courseResults.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 px-2 mb-2">
                    <MapPin className="h-3.5 w-3.5" style={{ color: theme.iconDim }} />
                    <span className="text-xs font-medium uppercase tracking-wide" style={{ color: theme.textDim }}>Clubs & Courses</span>
                  </div>
                  <div className="space-y-0.5">
              ...
              {businessResults.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 px-2 mb-2">
                    <Building className="h-3.5 w-3.5" style={{ color: theme.iconDim }} />
                    <span className="text-xs font-medium uppercase tracking-wide" style={{ color: theme.textDim }}>Business Profiles</span>
                  </div>
                  <div className="space-y-0.5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3 px-2">
                    <div className="flex items-center gap-2">
                      <Clock className="h-3.5 w-3.5" style={{ color: theme.iconDim }} />
                      <span className="text-xs font-medium uppercase tracking-wide" style={{ color: theme.textDim }}>Recent</span>
                    </div>
                    <button
              ...
              {popularItems.length > 0 && (
                <div>
                  <div className="flex items-center gap-2 px-2 mb-3">
                    <TrendingUp className="h-3.5 w-3.5" style={{ color: theme.iconDim }} />
                    <span className="text-xs font-medium uppercase tracking-wide" style={{ color: theme.textDim }}>Popular</span>
                  </div>
                  <div className="space-y-0.5">
...
const ResultRow: React.FC<ResultRowProps> = ({ item, isActive, onClick, query, getInitials, theme }) => {
    ...
    >
      ...
      >
        {item.image ? (
          ...
          <span className="text-xs font-medium" style={{ color: theme.textDim }}>{getInitials(item.title)}</span>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs truncate" style={{ color: theme.textDim }}>{item.subtitle}</div>
      </div>

      {/* Chevron */}
      <ChevronRight 
        className="h-4 w-4 flex-shrink-0 transition-colors group-hover:opacity-80" 
        style={{ color: theme.iconDim, opacity: 0.5 }}
      />
    </button>
  );
};
