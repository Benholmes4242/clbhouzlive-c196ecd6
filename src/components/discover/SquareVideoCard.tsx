...
interface SquareVideoCardProps {
  item: ExploreContentItem;
  onMediaClick?: (item: ExploreContentItem) => void;
}

const SquareVideoCard: React.FC<SquareVideoCardProps> = ({ item, onMediaClick }) => {
  ...
  return (
    <div ref={containerRef} className="w-full">
      <div className="relative w-full aspect-square overflow-hidden rounded-lg shadow-md">
        ...
        {/* Glass Overlay */}
        <div className="absolute inset-x-0 bottom-0 bg-[linear-gradient(180deg,rgba(0,0,0,0)_0%,rgba(0,0,0,0.35)_60%,rgba(0,0,0,0.45)_100%)] backdrop-blur-[6px] pt-12 pb-3 px-3 z-20">
          {/* Course Tag */}
          ...
          {/* Caption */}
          <p className="text-[14px] leading-[18px] font-semibold text-white line-clamp-1 mb-2 drop-shadow-md">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[12px] text-white/85 font-medium truncate">@{item.user?.username || item.user?.name || 'unknown'}</span>
          </div>
        </div>
      </div>
    </div>
