...
const PostMetadata = ({ title, description, user, onUserClick, className }: PostMetadataProps) => {
  ...
  return (
    <div 
      ...
      style={{
        bottom: 'calc(var(--bottom-nav-height, 72px) + env(safe-area-inset-bottom, 0px) + 24px - var(--chrome-bottom-shift, 0px))', // aligned to share icon baseline (nav + safe area + gap)
      }}
    >
      {/* User Profile */}
