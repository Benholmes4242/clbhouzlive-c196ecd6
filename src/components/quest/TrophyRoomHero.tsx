...
export const TrophyRoomHero: React.FC<TrophyRoomHeroProps> = ({
  ...
  const heroTitle = useMemo(() => {
    if (!currentTier) return t('quest.hero.beginJourney');
    if (!nextMilestone) return t('quest.hero.grandSlamAchieved');
    return t('quest.hero.tierAchieved', { tier: tierName });
  }, [nextMilestone, currentTier, tierName, t]);

  const progressLabel = useMemo(() => {
    if (!nextMilestone) return null;
    if (isOwnProfile) {
      return t('quest.hero.toGoParen', { count: remaining });
    }
    return t('quest.hero.ofPlayed', { played: totalPlayed, total: nextThreshold });
  }, [isOwnProfile, remaining, totalPlayed, nextThreshold, nextMilestone, t]);
        ...
        {nextMilestone ? (
          <div className="relative flex flex-col items-center">
            ...
            <span className="text-xs text-muted-foreground mt-1">
              {t('quest.hero.toShort', { label: nextMilestone.shortLabel })}
            </span>
          </div>
        ) : (
          <div className="relative flex flex-col items-center">
            ...
            <span className="text-xs text-muted-foreground mt-1">{t('quest.hero.complete')}</span>
          </div>
        )}
          ...
          {nextMilestone ? (
            <>
              <div className="flex items-baseline justify-end gap-1">
                ...
                <span className="text-xs font-medium text-muted-foreground">{t('quest.hero.toGo')}</span>
              </div>
              <p className="text-xs text-muted-foreground mt-0.5">
                {t('quest.hero.toClub', { club: nextClubName })}
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{t('quest.hero.coursesPlayed')}</p>
            </>
          )}
        </div>
      </motion.div>
      ...
      >
        ...
        {nextMilestone && (
          <p className="text-xs text-muted-foreground mt-1.5 text-center tabular-nums">
            <Trans
              i18nKey="quest.hero.progressLine"
              ns="achievements"
              values={{ played: totalPlayed, total: nextThreshold, club: nextClubName }}
              components={{
                0: <span />,
                1: <span />,
