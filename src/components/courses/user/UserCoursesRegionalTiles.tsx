              ...
              {sortOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 text-base font-medium whitespace-nowrap px-2 py-1.5 shadow-lg shadow-black/10 transition-colors ${
                      sortBy === option.key 
                ...
                {sortOptions.map((option) => {
                  ...
                  return (
                    <button
                      key={option.key}
                      onClick={() => onSortChange(option.key)}
                      className={`flex items-center gap-2 text-base font-medium whitespace-nowrap px-2 py-1.5 shadow-lg shadow-black/10 transition-colors flex-shrink-0 ${
                        sortBy === option.key 
                          ? "bg-white/20 backdrop-blur-sm border border-white/30 text-white" 
                          : "bg-white/20 backdrop-blur-sm border border-white/30 text-white"
                      }`}
                  ...
                  <Tooltip key={tile.key}>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        className={`flex items-center justify-center gap-2 cursor-pointer transition-colors whitespace-nowrap px-2 py-1.5 shadow-lg shadow-black/10 text-base font-medium ${
                  ...
                  {tiles.map((tile) => (
                    <Tooltip key={tile.key}>
                      <TooltipTrigger asChild>
                        <button
                          onClick={() => onFilterChange(activeFilter === tile.key ? null : tile.key)}
                          className={`flex items-center gap-2 cursor-pointer transition-colors whitespace-nowrap flex-shrink-0 px-2 py-1.5 shadow-lg shadow-black/10 text-base font-medium ${
                            activeFilter === tile.key 
                              ? 'bg-white/20 backdrop-blur-sm border border-white/30 text-white' 
                              : 'bg-white/20 backdrop-blur-sm border border-white/30 text-white'
                          }`}
