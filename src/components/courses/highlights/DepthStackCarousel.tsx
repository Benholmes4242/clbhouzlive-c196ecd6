    ...
    <div className="relative w-full">
      ...
      >
        {carouselItems
          ...
          .map((item, index) => {
            // Calculate responsive width based on visible cards - ensuring proper gaps
            const getCardWidth = () => {
              if (typeof window === 'undefined') return 'w-80'; // SSR fallback
              
              const width = window.innerWidth;
              if (width <= 430) return 'w-[calc(90vw-12px)]'; // Mobile: single card with small gap
              if (width <= 768) return 'w-[340px]'; // Small tablet: fixed width
              if (width <= 1024) return 'w-[380px]'; // Large tablet: fixed width
              return 'w-[420px]'; // Desktop: fixed width for proper scrolling
            };
          ...
          {carouselItems.filter(item => item.videoUrl).map((_, index) => (
            <button
              ...
              onClick={() => {
                const container = containerRef.current;
                if (container) {
                  const cardWidth = isMobile ? container.offsetWidth * 0.9 : 420; // Match fixed widths
