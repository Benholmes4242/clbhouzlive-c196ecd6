...
const MobileGamificationLayout: React.FC<MobileGamificationLayoutProps> = ({
  ...
  return (
    <>
      {/* Mobile Trophy Timeline with Progress Bar - Scroll together */}
      <div className="md:hidden relative z-10 py-4">
        <div className="overflow-x-auto scrollbar-hide">
          <div className="relative px-4 pb-4 pt-2" style={{ minWidth: '800px' }}>
            {/* Progress Line - positioned relative within scrollable container */}
            <div className="relative top-4 h-3 bg-gray-200 rounded-full overflow-hidden shadow-sm mb-8">
            ...
            {/* Trophies positioned at specific points on the progress bar */}
            <div className="relative">
              {globalTrophies.map((trophy, index) => {
                // Calculate position based on required courses (0-300 range)
                const position = (trophy.requiredCourses / 300) * 100;
                
                return (
                  <div 
                    key={trophy.id} 
                    className="absolute flex flex-col items-center space-y-2 cursor-pointer hover:scale-110 transition-all duration-300 hover:-translate-y-1"
                    style={{ 
                      left: `${position}%`,
                      top: '-60px', // Position above the progress bar
                      transform: 'translateX(-50%)', // Center the trophy on its position
