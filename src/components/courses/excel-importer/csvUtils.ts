...
const parseCSVContent = (text: string): { data: ExcelCourseData[], debug: DebugInfo } => {
...
export const parseExcelFile = async (file: File): Promise<{ data: ExcelCourseData[], debug: DebugInfo }> => {
  return new Promise((resolve, reject) => {
    ...
    // For now, we'll only support CSV and text files
    // Excel files (.xlsx, .xls) require a specialized library
    if (fileExtension === 'xlsx' || fileExtension === 'xls') {
      reject(new Error('Excel files (.xlsx, .xls) are not yet supported. Please export your Excel file as CSV and try again.'));
      return;
    }
