                ...
                {parRows.map((row) => (
                  <div key={row.par} style={{ display: 'grid', gridTemplateColumns: '52px 1fr 34px 34px', gap: 10, alignItems: 'center' }}>
                    ...
                    <span style={{ position: 'relative', display: 'block', height: 7, borderRadius: BAR_RADIUS, background: A.TRACK }}>
                      ...
                      {row.fieldCount > 0 ? <i style={{ position: 'absolute', left: `${Math.max(0, row.field) / parScale * 100}%`, top: -2, width: 2, height: 11, borderRadius: 1, background: inkWithAlpha(SURFACE.dark.ink, 0.55) }} /> : null}
                    </span>
            ...
            <section style={{ padding: '34px 20px 0' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, alignItems: 'end', height: 112, marginTop: 14 }}>
                {thirds.map((value, index) => {
                  ...
                  return <div key={thirdLabels[index]} style={{ display: 'flex', height: '100%', flexDirection: 'column', justifyContent: 'flex-end', textAlign: 'center' }}><span style={{ ...FIGURE, fontSize: 15, fontWeight: 700, color: worst ? A.RED : A.INK }}>{signed(value)}</span><span style={{ display: 'block', height: `${Math.max(4, Math.max(0, value) / thirdsMax * 70)}px`, marginTop: 7, borderRadius: `${BAR_RADIUS}px ${BAR_RADIUS}px 0 0`, background: worst ? A.RED : inkWithAlpha(SURFACE.dark.ink, 0.22) }} /><span style={{ ...ABOUT_KICKER, marginTop: 7 }}>{index === 0 ? '1-6' : index === 1 ? '7-12' : '13-18'}</span></div>;
                })}
              </div>
