...
import { useVideosRecentSearches } from './hooks/useVideosRecentSearches';
...
export function VideosSearchOverlay({ isOpen, onClose, userId }: VideosSearchOverlayProps) {
  const [inputValue, setInputValue] = useState('');
  const [activeQuery, setActiveQuery] = useState('');
  const inputRef = useRef<HTMLInputElement>(null);
  const debounceRef = useRef<ReturnType<typeof setTimeout>>();

  const { searches, addSearch, removeSearch, clearAll } = useVideosRecentSearches();
