...
interface VideosAutoplayProps {
  posts: FeedPost[];
  feedRef: React.RefObject<HTMLElement>;
}

export function VideosAutoplay({ posts, feedRef }: VideosAutoplayProps) {
  ...
  const detachSlot = useCallback((slot: number) => {
    ...
    if ((video as any)._videosCanPlayHandler) {
      video.removeEventListener('canplay', (video as any)._videosCanPlayHandler);
      (video as any)._videosCanPlayHandler = null;
    }
  ...
  const attachToCard = useCallback(async (slot: number, cardIndex: number, post: FeedPost, mediaEl: HTMLElement) => {
    ...
    (video as any)._videosCanPlayHandler = onCanPlay;
