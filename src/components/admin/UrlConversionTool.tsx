...
const UrlConversionTool: React.FC = () => {
  ...
  const handleConvertUrls = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast({
          title: "URL Conversion Complete",
          description: `Successfully updated ${data.totalUpdated} media URLs to R2`,
        });
      } else {
        toast({
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Convert all database URLs from Supabase storage to Cloudflare R2 format.
          This fixes broken media after migration.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            This will update URLs in the following tables:
          </p>
