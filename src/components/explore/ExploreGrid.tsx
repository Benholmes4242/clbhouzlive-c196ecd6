...
const ExploreGrid: React.FC<ExploreGridProps> = ({ 
  ...
  // Function to create dynamic mobile grid layout
  const createDynamicMobileLayout = (content: ExploreContentItem[]) => {
    const layoutItems = [];
    let contentIndex = 0;
    let specialCardCount = 0;
    let lastSpecialType: 'wide' | 'tall' | null = null;
    
    while (contentIndex < content.length) {
      const remainingItems = content.length - contentIndex;
      
      // Calculate if we should place a special card (every 6-8 squares)
      const regularCardsSinceSpecial = layoutItems.filter(item => item.cardType === 'square').length - specialCardCount * 6;
      const shouldPlaceSpecial = regularCardsSinceSpecial >= 6 && remainingItems >= 3;
      
      if (shouldPlaceSpecial && specialCardCount < Math.ceil(content.length * 0.3)) {
        // Look ahead to find the best aspect ratio match
        let bestIndex = -1;
        let bestType: 'wide' | 'tall' = 'wide';
        
        // Search next 8 items for best aspect ratio match
        for (let i = contentIndex; i < Math.min(contentIndex + 8, content.length); i++) {
          const detectedRatio = detectAspectRatio(content[i]);
          
          // Prioritize alternating between wide and tall
          if (detectedRatio === 'wide' && lastSpecialType !== 'wide') {
            bestIndex = i;
            bestType = 'wide';
            break;
          } else if (detectedRatio === 'tall' && lastSpecialType !== 'tall') {
            bestIndex = i;
            bestType = 'tall';
            break;
          } else if (bestIndex === -1 && (detectedRatio === 'wide' || detectedRatio === 'tall')) {
            bestIndex = i;
            bestType = detectedRatio as 'wide' | 'tall';
          }
        }
        
        // If no good match found, use current item and determine type by alternation
        if (bestIndex === -1) {
          bestIndex = contentIndex;
          bestType = lastSpecialType === 'wide' ? 'tall' : 'wide';
        }
        
        // Add regular cards before the special card
        while (contentIndex < bestIndex) {
          layoutItems.push({
            type: 'regular',
            item: content[contentIndex],
            index: contentIndex,
            shouldAutoplay: content[contentIndex].type === 'video' && contentIndex % 5 === 0,
            cardType: 'square'
          });
          contentIndex++;
        }
        ...
        layoutItems.push({
          type: 'special',
          item: content[bestIndex],
          index: bestIndex,
          shouldAutoplay: content[bestIndex].type === 'video',
          cardType: bestType,
          specialType: bestType
        });
        
        specialCardCount++;
        lastSpecialType = bestType;
        contentIndex = bestIndex + 1;
        
      } else {
        // Add regular square card
        layoutItems.push({
          type: 'regular',
          item: content[contentIndex],
          index: contentIndex,
  ...
  if (isDiscoverPage) {
    ...
    // Use dynamic mobile layout on mobile, otherwise use existing layout
    if (isMobile) {
      const dynamicLayoutItems = createDynamicMobileLayout(filteredContent);
      
      // Helper functions for media navigation (same as before)
      ...
      // Create touch handlers for swipe detection (same as before)
      ...
      return (
        <>
          {/* Dynamic Mobile Grid Layout */}
          <div className="grid grid-cols-3 gap-0.5 auto-rows-[1fr]">
            {dynamicLayoutItems.map((layoutItem, index) => {
              ...
              // Determine grid classes based on card type
              let gridClasses = 'aspect-square'; // Default
              if (layoutItem.cardType === 'wide') {
                gridClasses = 'col-span-2 aspect-[2/1]';
              } else if (layoutItem.cardType === 'tall') {
                gridClasses = 'row-span-2 aspect-[1/2]';
              }

              return (
                <div
                  key={`dynamic-${layoutItem.item.id}-${index}`}
                  className={`${gridClasses} relative overflow-hidden cursor-pointer group`}
                  style={{ borderRadius: '0px' }}
                ...
                >
                  ...
                  {/* Context Label for special cards */}
                  {!hideBadges && layoutItem.type === 'special' && (
                  ...
                  {/* User info for special cards */}
                  {layoutItem.type === 'special' && (
    ...
    // Desktop layout - use existing complex layout
  ...
  return (
    <>
      {/* Dynamic Mobile Grid Layout for Profile Activity Feed */}
      {isMobile ? (
        <>
          <div className="grid grid-cols-3 gap-0.5 auto-rows-[1fr]">
            {(() => {
              ...
              return dynamicLayoutItems.map((layoutItem, index) => {
                const hasMultipleMedia = layoutItem.item.media && layoutItem.item.media.length > 1;
                const currentMediaIndex = mediaIndices[layoutItem.item.id] || 0;
                const currentMedia = hasMultipleMedia ? layoutItem.item.media![currentMediaIndex] : null;
                ...
                return (
                  <div
                    key={`profile-dynamic-${layoutItem.item.id}-${index}`}
                    className={`${gridClasses} relative overflow-hidden cursor-pointer group`}
                    style={{ borderRadius: '0px' }}
                    onClick={() => onMediaClick?.(layoutItem.item)}
                    {...(hasMultipleMedia ? createTouchHandlers(layoutItem.item.id, layoutItem.item.media!.length) : {})}
        ...
        <>
          {/* Desktop: Instagram-style Grid Layout with Featured Cards */}
