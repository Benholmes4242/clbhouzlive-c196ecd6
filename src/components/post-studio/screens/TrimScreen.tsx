// TrimScreen — Full-bleed video preview with light chrome + dark canvas
...
export function TrimScreen() {
  ...
  return (
    <div className="flex-1 flex flex-col" style={{ background: '#F8FAFC', minHeight: 0, overflow: 'hidden' }}>
      {/* Header */}
      <header className="flex items-center justify-between px-4 shrink-0" style={{ paddingTop: 'max(env(safe-area-inset-top, 0px), 47px)', minHeight: 52 }}>
        ...
        <span style={{ fontSize: 15, fontWeight: 700, color: '#0F172A' }}>Trim</span>
        <motion.button
          whileTap={{ scale: 0.96 }}
      ...
      <div className="shrink-0" style={{
        ...
        <div className="flex items-center justify-between mb-3.5">
          <div>
            <span style={{ fontSize: 10, fontWeight: 600, color: 'rgba(15,23,42,0.40)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Start</span>
            <p style={{ fontSize: 18, fontWeight: 700, color: '#0F172A', fontVariantNumeric: 'tabular-nums' }}>{fmt(trimStart)}</p>
          </div>
          <span style={{ fontSize: 11, color: 'rgba(15,23,42,0.45)' }}>Drag handles to trim</span>
          <div className="text-right">
            <span style={{ fontSize: 10, fontWeight: 600, color: 'rgba(15,23,42,0.40)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>End</span>
            <p style={{ fontSize: 18, fontWeight: 700, color: '#0F172A', fontVariantNumeric: 'tabular-nums' }}>{fmt(trimEnd)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <span style={{ fontSize: 11, color: 'rgba(15,23,42,0.45)' }}>Full video: {fmt(totalDuration)}</span>
