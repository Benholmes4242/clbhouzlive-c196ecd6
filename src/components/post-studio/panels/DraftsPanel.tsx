  ...
  return (
    <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', ...SPRING.panel }}
      ...
      <div className="flex items-center justify-between px-4 pb-2">
        <div><p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[1.5px]">YOUR DRAFTS</p>
        <h3 className="text-sm font-semibold text-foreground">Drafts</h3></div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {isLoading && (
          <div className="grid grid-cols-2 gap-3">
            {[1, 2, 3, 4].map((i) => <div key={i} className="aspect-square rounded-xl bg-muted clb-shimmer-dark" />)}
          </div>
        )}
        {!isLoading && drafts.length === 0 && (
          <div className="text-center py-10">
            <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[1.5px]">YOUR DRAFTS</p>
            <p className="text-foreground font-semibold mt-1">No drafts yet</p>
            <p className="text-muted-foreground text-xs mt-1">Start a post and save it to come back later</p>
          </div>
        )}
        {!isLoading && drafts.length > 0 && (
          <div className="grid grid-cols-2 gap-3">
              ...
              return (
                <button key={draft.id} onClick={() => handleLoadDraft(draft)}
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-2">
                    ...
                    <p className="text-white/60 text-[9px] mt-0.5">{new Date(draft.updatedAt).toLocaleDateString()}</p>
                  </div>
