...
export function CourseTagPanel() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      } catch (err) { console.error('[CourseTagPanel] search error:', err); }
  ...
  return (
    <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', ...SPRING.panel }}
      ...
      <div className="flex items-center justify-between px-4 pb-2">
        <div><p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[1.5px]">TAG A COURSE</p>
        <h3 className="text-sm font-semibold text-foreground">Tag a Course</h3></div>
      ...
      {state.taggedCourses.length > 0 && (
        <div className="flex flex-wrap gap-2 px-4 pb-3">
          {state.taggedCourses.map((course) => (
            <span key={course.courseId} className="inline-flex items-center gap-1 bg-primary/15 text-primary text-xs px-2.5 py-1 rounded-full border border-primary/20">
              ⛳ {course.courseName}
              <button onClick={() => handleRemove(course.courseId)} className="w-4 h-4 flex items-center justify-center"><X className="w-3 h-3" /></button>
            </span>
          ))}
        </div>
      )}
      <div className="px-4 pb-3">
        <div className="flex items-center gap-2 bg-muted rounded-xl px-3 py-2.5 border border-border/50 focus-within:border-primary/50 transition-colors">
          ...
          <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search courses…"
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {!isSearching && results.length === 0 && query.length >= 2 && (<p className="text-center text-muted-foreground text-sm py-6">No courses found</p>)}
        {results.map((course) => {
          const isTagged = state.taggedCourses.some((c) => c.courseId === course.id);
          return (
            <button key={course.id} onClick={() => handleSelect(course)} disabled={isTagged}
              className="w-full flex items-center gap-3 py-3 border-b border-border/30 last:border-0 min-h-[52px] disabled:opacity-40 border-l-2 border-l-transparent hover:border-l-primary/50 transition-colors">
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-muted-foreground text-xs">{[course.region, course.country].filter(Boolean).join(', ')}</p>
              </div>
              {isTagged && <span className="text-[10px] text-primary bg-primary/10 px-2 py-0.5 rounded-full">Added</span>}
            </button>
          );
        })}
      </div>
