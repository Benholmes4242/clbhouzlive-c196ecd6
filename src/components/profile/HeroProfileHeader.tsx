      ...
      {isMobile ? (
        <div className="relative -mt-16 bg-white">
          <section className="relative w-full overflow-visible">
            ...
            {/* COMPACT GLASS PANEL with overhanging mini card */}
            <section
              ref={profileCardRef}
              className="relative z-20 mx-4 rounded-2xl border border-white/35 bg-white/10 backdrop-blur-xl shadow-[0_10px_30px_rgba(0,0,0,0.15)] px-4 py-3"
            ...
            >
              <div className="relative">
                ...
                <div 
                  ref={miniCardRef}
                  className="absolute top-[-25%] right-0 rounded-lg border border-white/40 bg-white/20 backdrop-blur-sm shadow-lg overflow-hidden cursor-pointer hover:bg-white/30 transition-all duration-200 z-20"
                ...
                {/* Left-aligned content with space for overhanging card */}
                <div className="pr-[calc(var(--mini-w)+16px)]">
                  {/* User Name & Handle - Left aligned */}
                  <div ref={nameBlockRef} className="text-left mb-4">
                    <h1 
                      className="displayName font-bold leading-tight text-[length:var(--fs-display)] m-0 mb-1 text-gray-900"
                      title={displayName}
                    >
                      {displayName}
                    </h1>
                    <div className="handle opacity-70 text-[length:var(--fs-handle)] m-0 text-gray-700">
                      {homeClub || 'Golf Club'}
                    </div>
                  </div>

                  {/* Golf Club + Handicap Row - Horizontal layout */}
                  <div 
                    ref={metaRowRef}
                    className="clubHandicapRow flex justify-between items-center gap-6"
                  >
                    {/* Golf Club - Left aligned */}
                    <div className="flex-1 text-left">
                      <div className="statTitle text-xs opacity-70 mb-1 text-gray-600">Golf Club</div>
                      <div className="statValue clubValue text-sm font-semibold text-gray-900 truncate">
                        {homeClub || 'Not specified'}
                      </div>
                    </div>
                    
                    {/* Handicap - Right aligned */}
                    <div className="text-right">
                      <div className="statTitle text-xs opacity-70 mb-1 text-gray-600">Handicap</div>
                      <div className="statValue handicapValue text-lg font-bold text-gray-900">
                        {handicap || '—'}
                      </div>
                    </div>
                  </div>
