...
interface AchievementContext {
  ...
  /** Unused — kept for back-compat with TrophiesSheetMount call signature. */
...
function findFirstCrossDown(history: HandicapPoint[], threshold: number): string | null {
  ...
  return sorted.find((p) => p.handicap_index < threshold)?.observed_at ?? null;
}
...
export function computeAchievements(ctx: AchievementContext): Achievement[] {
  ...
  out.push({
    ...
    achieved_at: ctx.connectionCreatedAt ?? null,
  });
  ...
  {
    const sorted = [...ctx.scores].sort(
      (a, b) => new Date(a.play_date).getTime() - new Date(b.play_date).getTime()
    );
    const firstDate = sorted[0]?.play_date ?? null;
    out.push({
 ...
 * Locked binary first, then in-progress lists by progress descending.
 */
