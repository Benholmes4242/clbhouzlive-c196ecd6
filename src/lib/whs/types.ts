...
export interface Achievement {
  /** Stable identifier — matches AchievementType. */
  id: string;
  /** Type slug. */
  type: AchievementType;
  /** Display title (15px, top of row). */
  title: string;
  /** Always-visible description (11.5px, below title). Same voice in every state. */
  description: string;
  /** Icon name for lucide-react lookup. */
  icon_name: string;
  /** Category for grouping. */
  category: AchievementCategory;
  /** Trophy kind — drives render path. */
  kind: AchievementKind;

  // ── Binary trophies ─────────────────────────────────────────────
  /** For binary: true when earned. Counter and list types ignore. */
  earned?: boolean;
  /** ISO date when earned (binary only). null if locked. */
  achieved_at?: string | null;

  // ── Counter trophies ────────────────────────────────────────────
  /** Running total. Always present, may be 0. */
  count?: number;
  /** Label under the counter number ("total", "rounds", "countries", "courses"). */
  count_label?: string;

  // ── List trophies ───────────────────────────────────────────────
  /** Number of unique courses played in this list. */
  list_played?: number;
...
export interface FriendRivalryHydrated extends FriendRivalry {
  ...
  rival_mobile_crop_width: number | null;
  rival_mobile_crop_height: number | null;
}
