...
export function useActiveGolfers({ 
  ...
  const useMockData = true; // Enabled: returns only Andrew Yetzes + Gary Martyn
  ...
  const { data: golfers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      // If mock mode is enabled, return ONLY mock data (ignore real users completely)
      ...
      // Otherwise, fetch real golfers from RPC
