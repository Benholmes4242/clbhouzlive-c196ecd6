...
export function useProfileSave(userId) {
    const queryClient = useQueryClient();
    const [isSaving, setIsSaving] = useState(false);
    const save = async (form, opts = {}) => {
        ...
        try {
            ...
            if (form.profilePhotoBlob) {
                const file = new File([form.profilePhotoBlob], `profile-${Date.now()}.jpg`, { type: 'image/jpeg' });
            ...
            if (form.headerPhotoBlob) {
                const file = new File([form.headerPhotoBlob], `header-${Date.now()}.jpg`, { type: 'image/jpeg' });
            ...
            if (updatePayload.display_name == null)
                delete updatePayload.display_name;
            if (updatePayload.first_name == null)
                delete updatePayload.first_name;
            if (updatePayload.last_name == null)
            ...
            await Promise.all(INVALIDATE_KEYS.map(key => queryClient.invalidateQueries({ queryKey: [key], refetchType: 'all' })));
