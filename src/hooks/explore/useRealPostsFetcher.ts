...
export const fetchClubhouseExploreShorts = async (
  ...
  // Duplicate the fetch logic here for standalone use
  const fetchStart = performance.now();
  console.log(`[${fetchStart.toFixed(2)}ms] [Prefetch] Starting standalone prefetch`);
  ...
  try {
    ...
    if (error || !postsData || postsData.length === 0) {
      console.log(`[${performance.now().toFixed(2)}ms] [Prefetch] No data or error`);
      return [];
    }
    ...
    const formattedPosts = postsData
      ...
      .map(post => {
        ...
        return {
          ...
          type: 'video' as const,
          src: firstMedia.media_url,
          videoUrl: firstMedia.media_url,
          thumbnailUrl: firstMedia.poster_url || getStreamPoster(firstMedia.media_url),
          title: post.content || '',
          durationSeconds: firstMedia.duration_seconds || 0,
          ...
          tagged_entities: (post.post_tags || [])
            .filter((tag: any) => tag.taggable_entities)
            .map((tag: any) => tag.taggable_entities),
        };
      });

    console.log(`[${performance.now().toFixed(2)}ms] [Prefetch] Completed with ${formattedPosts.length} posts`);
    return formattedPosts;
  } catch (error) {
    console.error(`[${performance.now().toFixed(2)}ms] [Prefetch] Error:`, error);
    return [];
  }
};