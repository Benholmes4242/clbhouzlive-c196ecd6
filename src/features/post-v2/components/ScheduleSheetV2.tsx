    ...
    <BottomSheet open={open} title="Schedule" onClose={onClose}>
      <div style={{ padding: '4px 16px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {value && (
            <button onClick={clear} style={{ flex: 1, background: inkWithAlpha(SURFACE.dark.ink, 0.06), border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '12px', fontSize: 14, cursor: 'pointer', color: CT.ink }}>Clear</button>
          )}
          <button
            onClick={apply}
            disabled={isPast}
