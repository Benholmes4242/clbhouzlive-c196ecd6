...
export function OverviewNews() {
  ...
  return <section><OverviewSectionHead title={t('overview.news.title')} action={t('overview.news.all')} onAction={() => navigate('/tourhub?tab=news')} /><article style={{ fontFamily: FONT }}><button type="button" onClick={() => navigate(`/tour/news/${lead.slug}`)} style={{ width: '100%', padding: 0, border: 0, borderBottom: `1px solid ${WHITE_ALPHA_06}`, background: 'transparent', color: INK, textAlign: 'left', cursor: 'pointer' }}>{lead.image_url ? <img src={lead.image_url} alt="" loading="lazy" style={{ display: 'block', width: '100%', height: 190, objectFit: 'cover' }} /> : null}<span style={{ display: 'block', padding: '12px 24px 14px' }}><StoryMeta story={lead} /><span style={{ display: '-webkit-box', marginTop: 6, overflow: 'hidden', WebkitBoxOrient: 'vertical', WebkitLineClamp: 3, fontSize: 18, lineHeight: 1.22, fontWeight: 800 }}>{lead.headline}</span></span></button>{rows.map((story, index) => <NewsRow key={story.id} story={story} last={index === rows.length - 1} onOpen={() => navigate(`/tour/news/${story.slug}`)} />)}</article></section>;
}