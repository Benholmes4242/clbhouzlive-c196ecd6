...
import { DANGER, FONT, INK, INK_FAINT, INK_MUTE, TREND_UP, WHITE_ALPHA_06 } from '../../_shared/tokens';
...
function RankingRow({ row, last, onOpen, showMovement }: { row: RankingsRow; last: boolean; onOpen?: () => void; showMovement: boolean }) {
  return <button type="button" disabled={!onOpen} onClick={onOpen} style={{ width: '100%', minHeight: 56, padding: '8px 24px', display: 'grid', gridTemplateColumns: showMovement ? '28px 34px minmax(0,1fr) 64px' : '28px minmax(0,1fr) 64px', alignItems: 'center', border: 0, borderBottom: last ? 'none' : `1px solid ${WHITE_ALPHA_06}`, background: 'transparent', color: INK, textAlign: 'left', fontFamily: FONT, cursor: onOpen ? 'pointer' : 'default' }}><span style={{ fontSize: 15, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{row.rank}</span>{showMovement ? <Movement value={row.movement} /> : null}<span style={{ minWidth: 0 }}><span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 14, fontWeight: 700 }}>{row.playerName}</span>{row.country ? <span style={{ display: 'block', marginTop: 2, fontSize: 11, color: INK_MUTE }}>{row.country.toUpperCase()}</span> : null}</span><span style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: 'rgba(248,250,252,0.8)', fontVariantNumeric: 'tabular-nums' }}>{row.points == null ? '' : Math.round(row.points).toLocaleString()}</span></button>;
}
/**
 * Data-driven, never a board list: the movement track exists only when at
 * least one of the five rows actually drawn carries a value. A board whose
 * feed starts sending position_change gains the column with no code change.
 */
