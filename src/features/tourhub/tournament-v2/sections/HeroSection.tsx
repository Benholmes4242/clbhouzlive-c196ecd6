...
export function HeroSection({ meta, state, imageUrl, tourCode, contest, fieldCount }: Props) {
  ...
  const needsInitials = teamNamesNeedInitials(contest.pack.map((row) => row.entry));
  const leaderName = contest.leader ? resolveBoardEntity(contest.leader, needsInitials).prose : null;
  const completedChampion = state === 'completed'
    ? resolveChampion(contest.pack.map((row) => row.entry), meta)
    : null;
  ...
  // The verdict is a claim about who is winning. Stroke/team boards support it;
  // cup/match do not, and a tied team top names nobody.
  const strokeBoard = hasStrokeBoard(meta.event_type);
  let verdict: string | null = null;
  if (!strokeBoard || (state === 'completed' && !completedChampionName)) {
    verdict = null;
  ...
  } else if (state === 'completed' && completedChampionName) {
    if (contest.sharedLead) verdict = t('tournament.hero.verdict.completedPlayoff', { winner: completedChampionName });
    else if (contest.margin != null && contest.margin >= 1) verdict = t('tournament.hero.verdict.completedMargin', { winner: completedChampionName, margin: contest.margin });
    ...
    <div style={{ fontFamily: FONT, color: INK }}>
      <div style={{ position: 'relative', minHeight: TOUR_HERO_PHOTO_H, margin: 0, borderRadius: 0, overflow: 'hidden', paddingTop: 20, background: heroCanonBackground(imageUrl, FALLBACK_BG, IMAGE_FOCAL), display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        <div style={{ padding: '8px 14px 14px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, minHeight: 28, padding: '0 10px', borderRadius: 999, ...DARK_CHROME_GLASS_MATERIAL, ...CAPS_10, color: INK, letterSpacing: cjk ? 0 : CAPS_10.letterSpacing, textTransform: cjk ? 'none' : CAPS_10.textTransform }}>
