...
import { AMBER, FONT, INK, INK_FAINT, INK_MUTE, WHITE_ALPHA_06 } from '../_shared/tokens';
...
export function SeasonStatsIndex() {
  ...
  return <TourPageShell title={t('leaders.index.title')} backFallback="/tourhub?tab=leaderboards" leftAccessory={lens}>
    <main style={{ minHeight: '100vh', paddingTop: 'var(--tour-header-h, 0px)', paddingBottom: NAV_CLEARANCE, background: A.CANVAS, fontFamily: FONT }}>
      <header style={{ padding: '28px 24px 20px', borderBottom: `1px solid ${WHITE_ALPHA_06}` }}><p style={{ margin: 0, color: AMBER, fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{t('leaders.index.kicker')}</p><h1 style={{ margin: '7px 0 0', color: INK, fontSize: 32, lineHeight: 1, fontWeight: 900 }}>{t('leaders.index.title')}</h1><p style={{ margin: '10px 0 0', color: INK_MUTE, fontSize: 12 }}>{t('leaders.index.standfirst')}</p><div style={{ marginTop: 18 }}><CatalogueSearchField value={query} onChange={setQuery} placeholder={t('leaders.index.search')} /></div></header>
      {isLoading ? <p style={{ padding: 24, color: INK_MUTE }}>{t('leaders.index.loading')}</p> : !visible.length ? <TourHubEmptyState variant="leaderboard" /> : GROUPS.map((group) => { const grouped = visible.filter((category) => category.group === group); if (!grouped.length) return null; return <section key={group}><h2 style={{ margin: 0, padding: '24px 24px 8px', color: INK_MUTE, fontSize: 10, fontWeight: 850, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{t(`leaders.index.groups.${group}`)}</h2>{grouped.map((category, index) => { const leader = category.rows[0]; const country = leader?.countryCode || leader?.country; return <button key={category.key} type="button" className="active:bg-white/[0.06] transition-colors" onClick={() => setOpenKey(category.key)} style={{ width: '100%', minHeight: 76, padding: '13px 24px', display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto 14px', gap: 12, alignItems: 'center', border: 0, borderBottom: index === grouped.length - 1 ? 'none' : `1px solid ${WHITE_ALPHA_06}`, background: 'transparent', textAlign: 'left', cursor: 'pointer' }}><span style={{ minWidth: 0 }}><span style={{ display: 'block', color: INK, fontSize: 14, fontWeight: 800 }}>{t(category.labelKey)}</span><span style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0, marginTop: 5 }}><span style={{ minWidth: 0, color: INK_MUTE, fontSize: 11, overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{leader?.name ?? ''}</span>{country ? <CountryFlag country={country} size="sm" /> : null}</span></span><span style={{ color: A.INK, fontSize: 17, fontWeight: 900, fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>{leader?.valueFormatted ?? ''}</span><ChevronRight size={14} color={INK_FAINT} aria-hidden /></button>; })}</section>; })}
    </main>
