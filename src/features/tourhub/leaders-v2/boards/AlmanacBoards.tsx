...
export function RaceStandingsModule({ category, onPlayerClick }: { category: LeaderCategoryDef; onPlayerClick: (row: LeaderRow) => void }) {
  ...
  return <section style={{ padding: '28px 24px 30px', borderBottom: `1px solid ${WHITE_ALPHA_06}` }}><p style={eyebrow}>{t('leaders.season.race')}</p><h2 style={{ ...title, marginTop: 7, fontSize: 28 }}>{t(category.labelKey)}</h2><p style={{ ...copy, marginTop: 8 }}>{t('leaders.season.raceTableStandfirst')}</p><div style={{ marginTop: 18, borderTop: `3px solid ${AMBER}` }}>{rows.map((row, index) => <button type="button" key={row.playerId || row.name} onClick={() => onPlayerClick(row)} style={{ width: '100%', minHeight: 72, padding: '12px 0', border: 0, borderBottom: index === rows.length - 1 ? 'none' : `1px solid ${WHITE_ALPHA_06}`, background: 'transparent', display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) auto', alignItems: 'center', columnGap: 12, textAlign: 'left', cursor: 'pointer' }}><span style={{ color: index === 0 ? AMBER : INK_MUTE, fontSize: 12, fontWeight: 850, fontVariantNumeric: 'tabular-nums' }}>{row.rankLabel}</span><span style={{ minWidth: 0, color: A.INK, fontSize: 15, fontWeight: index === 0 ? 850 : 750, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.name}</span><span style={{ textAlign: 'right' }}><span style={{ display: 'block', color: index === 0 ? AMBER : A.INK, fontSize: 18, fontWeight: 900, fontVariantNumeric: 'tabular-nums' }}>{row.valueFormatted}</span>{category.meaningfulBehind && row.behindFormatted ? <span style={{ display: 'block', marginTop: 3, color: INK_MUTE, fontSize: 9, fontWeight: 750 }}>+{row.behindFormatted}</span> : null}</span></button>)}</div></section>;
}
