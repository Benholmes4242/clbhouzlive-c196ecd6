...
function SectionHead({ category, titleKey }: { category: LeaderCategoryDef; titleKey?: string }) {
  const { t } = useTranslation('tourhub');
  return <header style={{ padding: '22px 24px 10px' }}>
    <p style={sectionKicker}>{t(category.shortKey)}</p>
    <h2 style={sectionTitle}>{t(titleKey ?? `leaders.season.sectionTitle.${category.key}`)}</h2>
  </header>;
}
...
export function TiedListModule({ category, onPlayerClick }: { category: LeaderCategoryDef; onPlayerClick: (row: LeaderRow) => void }) {
  ...
  return <section style={{ borderBottom: `1px solid ${WHITE_ALPHA_06}` }}><SectionHead category={category} /><div style={{ padding: '4px 24px 24px' }}>{leaders.map((row, index) => <CompactRow key={row.playerId || row.name} row={row} last={index === leaders.length - 1} onPlayerClick={onPlayerClick} />)}</div></section>;
}
