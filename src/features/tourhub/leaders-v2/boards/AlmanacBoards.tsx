...
function PlayerNameLabel({ row, align = 'left', fontSize = 14, fontWeight = 700 }: { row: LeaderRow; align?: 'left' | 'right'; fontSize?: number; fontWeight?: number }) {
  const country = row.countryCode || row.country;
  return <span style={{ minWidth: 0, display: 'flex', alignItems: 'center', justifyContent: align === 'right' ? 'flex-end' : 'flex-start', gap: 6 }}>
    <span data-player-name={row.name} style={{ minWidth: 0, color: A.INK, fontSize, fontWeight, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.name}</span>
    {country ? <CountryFlag country={country} size="sm" /> : null}
  </span>;
}
...
function CompactRow({ row, last, onPlayerClick }: { row: LeaderRow; last: boolean; onPlayerClick: (row: LeaderRow) => void }) {
  return <button type="button" onClick={() => onPlayerClick(row)} style={{ width: '100%', minHeight: 68, padding: '12px 0', border: 0, borderBottom: last ? 'none' : `1px solid ${WHITE_ALPHA_06}`, background: 'transparent', display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) auto', alignItems: 'center', columnGap: 10, textAlign: 'left', cursor: 'pointer' }}>
    ...
    <span style={{ minWidth: 0 }}><PlayerNameLabel row={row} /><span style={{ display: 'block', marginTop: 4, fontSize: 10, fontWeight: 700 }}><Movement value={row.movement} /></span></span>
...
export function RaceStandingsModule({ category, onPlayerClick }: { category: LeaderCategoryDef; onPlayerClick: (row: LeaderRow) => void }) {
  ...
  return <section data-season-module="race" style={{ borderBottom: `1px solid ${WHITE_ALPHA_06}` }}><SectionHead category={category} titleKey="leaders.season.raceTableTitle" /><p style={{ ...copy, padding: '0 24px' }}>{t('leaders.season.raceTableStandfirst')}</p><div style={{ margin: '18px 24px 30px' }}>{rows.map((row, index) => <button type="button" key={row.playerId || row.name} onClick={() => onPlayerClick(row)} style={{ width: '100%', minHeight: 72, padding: '12px 0', border: 0, borderBottom: index === rows.length - 1 ? 'none' : `1px solid ${WHITE_ALPHA_06}`, background: 'transparent', display: 'grid', gridTemplateColumns: '28px minmax(0,1fr) auto', alignItems: 'center', columnGap: 12, textAlign: 'left', cursor: 'pointer' }}><span style={{ color: index === 0 ? STATUS_LIVE : INK_MUTE, fontSize: 12, fontWeight: 800, ...figure }}>{row.rankLabel}</span><PlayerNameLabel row={row} fontSize={15} fontWeight={index === 0 ? 800 : 700} /><span style={{ textAlign: 'right' }}><span style={{ display: 'block', color: index === 0 ? STATUS_LIVE : A.INK, fontSize: 18, fontWeight: 800, ...figure }}>{row.valueFormatted}</span>{category.meaningfulBehind && row.behindFormatted ? <span style={{ display: 'block', marginTop: 3, color: INK_MUTE, fontSize: 9, fontWeight: 700 }}>+{row.behindFormatted}</span> : null}</span></button>)}</div></section>;
}
...
export function TiedListModule({ category, onPlayerClick }: { category: LeaderCategoryDef; onPlayerClick: (row: LeaderRow) => void }) {
  ...
  return <section data-season-module="tied" style={{ borderBottom: `1px solid ${WHITE_ALPHA_06}` }}><SectionHead category={category} titleKey="leaders.season.sectionTitle.tied" /><div style={{ padding: '4px 24px 24px' }}>{leaders.map((row, index) => <CompactRow key={row.playerId || row.name} row={row} last={index === leaders.length - 1} onPlayerClick={onPlayerClick} />)}</div></section>;
}
