    ...
    >
      ...
      >
        <img
          src={photoUrl}
          alt={player.playerName}
          className={`w-full h-full object-cover ${activeTour === 'LIV' ? 'object-bottom' : ''}`}
          loading="lazy"
          onError={(e) => { (e.target as HTMLImageElement).src = PLAYER_SILHOUETTE_URL; }}
        />
      </div>
...
className={`absolute inset-0 w-full h-full object-cover ${activeTour === 'LIV' ? 'object-bottom' : 'object-[center_10%]'}`}
                  loading="eager"
                  initial={{ scale: 1.06 }}
                  animate={{ scale: 1 }}
