...
// Icon mapping based on skill label
const getSkillIcon = (label: string): string => {
  const lower = label.toLowerCase();
  if (lower.includes('approach')) return '🎯';
  if (lower.includes('putting') || lower.includes('putt')) return '⛳';
  if (lower.includes('driving') || lower.includes('distance')) return '💨';
  if (lower.includes('iron')) return '🏌️';
  if (lower.includes('scrambl')) return '🔄';
  if (lower.includes('accuracy')) return '📐';
  return '⛳';
};
    ...
    <div className="bg-white rounded-[14px] border border-slate-200 p-4 shadow-sm">
      ...
      <div className="flex flex-col gap-2.5">
        {items.map((item, i) => (
          ...
          >
            {/* Skill icon */}
            <div className={`w-9 h-9 rounded-[10px] ${tierToIconBg[item.tier]} flex items-center justify-center text-base flex-shrink-0`}>
              {item.icon || getSkillIcon(item.label)}
            </div>
