...
interface RoundHistoryPillsProps {
  ...
  round4: number | null;
  currentRound: number;
  /** When true, highlights the last completed round pill with amber (for completed tournaments) */
  highlightFinal?: boolean;
}

export function RoundHistoryPills({ round1, round2, round3, round4, currentRound, highlightFinal }: RoundHistoryPillsProps) {
  const rounds = [
  ...
  // Find the last completed round for amber highlight
  const lastCompletedRound = highlightFinal
    ? Math.max(...rounds.filter(r => r.score !== null).map(r => r.roundNum), 0)
    : 0;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 4, width: 'fit-content' }}>
      {rounds.map(r => {
        ...
        const isFinalHighlight = highlightFinal && r.roundNum === lastCompletedRound;
