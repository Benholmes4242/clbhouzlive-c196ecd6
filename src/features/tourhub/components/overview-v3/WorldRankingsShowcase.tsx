 ...
 * Clean player photos with minimal text, snap scrolling
 */
...
function PlayerItem({ player, index }: { player: WorldRankedPlayer; index: number }) {
  ...
  return (
    <motion.button
      ...
      className="flex-shrink-0 w-[72px] text-center snap-start"
    ...
    >
      ...
      <div className="flex items-center justify-center gap-1 mb-0.5">
        <span className={cn(
          "text-xs font-bold",
          isNo1 ? "text-amber-500" : 
          isNo2 ? "text-slate-400" :
          isNo3 ? "text-amber-600" : "text-slate-400"
        )}>
          #{player.rank}
        </span>
        {isNo1 && <span className="text-xs">🏆</span>}
      </div>
    ...
    return (
      <section className="py-6 bg-[#F8FAFC]">
        <div className="px-4 mb-4">
          <div className="h-4 w-48 bg-slate-200 rounded animate-pulse mb-1" />
          <div className="h-6 w-32 bg-slate-200 rounded animate-pulse" />
        </div>
        <div className="flex gap-4 px-4 overflow-hidden">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="flex-shrink-0 w-[72px]">
              <div className="w-[72px] h-[72px] rounded-2xl bg-slate-200 animate-pulse mb-2" />
    ...
    <section className="py-6 bg-[#F8FAFC]">
      ...
      <div className="flex items-center justify-between px-4 mb-4">
        <div>
          ...
          <h2 className="text-xl font-bold text-slate-900">Top 10 Players</h2>
        </div>
        <Link 
