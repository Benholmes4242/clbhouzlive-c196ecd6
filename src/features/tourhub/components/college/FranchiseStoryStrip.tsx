 ...
 * "Chasing" tile removed per polish requirements.
 */
...
function StoryTileSkeleton({ delay = 0 }: { delay?: number }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3, delay }}
      className="flex flex-col p-4 rounded-xl bg-card border border-border shadow-sm"
    >
      <div className="flex items-center gap-1.5 mb-2">
  ...
  return (
    <div className={cn('grid grid-cols-2 gap-3', className)}>
      ...
      <StoryTile icon={TrendingUp} iconColor="text-emerald-500" title="This Week" delay={0}>
        <span className="text-sm font-semibold text-foreground leading-tight">{weekSummary}</span>
      </StoryTile>

      {/* Top Performer — with OWGR if available */}
      <StoryTile
        icon={Star}
      ...
      >
        ...
        {topAlumnus && (
          <span className="text-[11px] text-muted-foreground mt-1 truncate tabular-nums">
