  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        ...
        // BRIEF_NON_STROKE_GATE_2 §3 — generic fetch behind leaderboard/board
        // surfaces; cups and match play must never reach a leader-asserting read.
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('sr_tournaments')
        .select('*, season:sr_seasons(tour_name, tour_full_name)')
        // BRIEF_NON_STROKE_GATE_2 §3 — see the list query above.
        .eq('event_type', 'stroke')
        .eq('id', tournamentId)
        .single();
