...
export type RpcClient = { rpc: (fn: string, args: Record<string, unknown>) => Promise<{ data: unknown; error: unknown }> };
...
export function useHubMixedGrid(userId: string | undefined, filter: string = 'all') {
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const { data, error } = await (supabase as unknown as RpcClient).rpc('get_watch_mixed_grid_v2', {
        p_user_id: userId,
        p_filter: filter,
