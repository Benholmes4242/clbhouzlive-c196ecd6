/**
 * InviteToTripModal - Modal for inviting players to a trip
 * Similar to InviteToGameModal but uses invite_users_to_trip RPC
 */
...
import { useInviteSearchTrip, InvitableUser } from '../../hooks/useInviteSearchTrip';
...
export function InviteToTripModal({
  isOpen,
  onClose,
  tripId,
  tripName,
  onInviteSuccess,
}: InviteToTripModalProps) {
  const { searchInput, setSearchInput, users, isLoading } = useInviteSearchTrip(tripId);
  ...
  const handleInvite = useCallback(async (user: InvitableUser) => {
    ...
    try {
      const { error } = await supabase.rpc('invite_users_to_trip', {
        p_trip_id: tripId,
        p_user_ids: [user.id],
      });
            ...
            <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-border/50 flex-shrink-0">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  {tripName ? `Invite golfers to ${tripName}` : 'Search for golfers to invite'}
                </p>
              </div>
              <button
                onClick={onClose}
