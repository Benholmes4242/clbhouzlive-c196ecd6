...
export function RecentEchoScreen({ onClose }: RecentEchoScreenProps) {
  ...
  const { data: chatItems = [], isLoading: chatLoading, error: chatErr } = useEchoChatHistory();
  const { data: swingItems = [], isLoading: swingLoading, error: swingErr } = useSwingHistory();

  // Refetch when the sheet opens
  useEffect(() => {
    if (params.get('sheet') === 'recent-echo') {
      const t = setTimeout(() => {
        queryClient.invalidateQueries({ queryKey: ['echoChatHistory'] });
        queryClient.invalidateQueries({ queryKey: ['swingHistory'] });
      }, 250);
  ...
  if (chatErr) console.warn('[RecentEchoScreen] chat history error:', chatErr);
  if (swingErr) console.warn('[RecentEchoScreen] swing history error:', swingErr);
