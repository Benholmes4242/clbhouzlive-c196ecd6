 ...
 * Input-style button with one rotating suggestion
 */
...
export function HubEchoCompact() {
  ...
  const sendSuggestion = (e: React.MouseEvent) => {
    e.stopPropagation();
    navigateFromHub(`/hub/echo?msg=${encodeURIComponent(SUGGESTIONS[suggestionIdx])}`);
  };
  ...
  return (
    <Tile title="">
      <div className="h-full flex flex-col">
        ...
        <button
          onClick={openEcho}
          className="w-full h-10 rounded-xl px-3 text-left flex items-center gap-2 transition-all active:scale-[0.98]"
          style={{
        ...
        <button
          ...
          className="mt-2 text-[12px] text-left leading-snug line-clamp-2 transition"
