...
import { A, CAPTION, KICKER, LABEL, NUM, SANS, StatRow, Action, } from '@/features/courses/components/holes/analytical/tokens';
 ...
 */
function fmtBucketPct(pct, pctExact, count) {
    if (count > 0 && pct === 0) {
...
function Row({ title, subtitle, onClick, }) {
    return (<button type="button" onClick={onClick} className="active:scale-[0.99]" style={{
            display: 'flex',
            alignItems: 'center',
      ...
      <div style={{ minWidth: 0, flex: 1, paddingRight: 12 }}>
        ...
        {subtitle && (<div style={{ fontWeight: 500, fontSize: 11.5, color: A.DIM, marginTop: 2 }}>
            {subtitle}
          </div>)}
      </div>
...
function hasScoringData(course) {
    ...
    return (course.eagles_plus_count !== null &&
 ...
 */
function AnalyticsCourseRow({ course, delta, expanded, onToggle, onOpen, }) {
    const { t } = useTranslation('courses');
    const hasAvg = course.avg_to_par !== null && course.avg_to_par !== undefined;
    const avgVal = hasAvg ? course.avg_to_par : null;
    ...
    const deltaTone = deltaRounded == null || deltaRounded === 0 ? A.INK : deltaRounded > 0 ? HARDER : EASIER;
        ...
        ? [
            {
                ...
                pct: course.eagles_plus_pct,
                pctExact: course.eagles_plus_pct_exact,
                count: course.eagles_plus_count,
            },
            {
                key: 'birdies',
                bg: DIST_SEG_COLORS.birdie,
                label: t('yourCourses.pillBirdiesLong'),
                pct: course.birdies_pct,
                pctExact: course.birdies_pct_exact,
                count: course.birdies_count,
            },
            {
                key: 'pars',
                ...
                pct: course.pars_pct,
                pctExact: course.pars_pct_exact,
                count: course.pars_count,
            },
            {
                key: 'bogeys',
                bg: DIST_SEG_COLORS.bogey,
                label: t('yourCourses.pillBogeysLong'),
                pct: course.bogeys_plus_pct,
                pctExact: course.bogeys_plus_pct_exact,
                count: course.bogeys_plus_count,
            },
        ]
        : [];
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, minWidth: 0 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {hasScoring && (<span style={{
              ...
              {segs.map((s) => (<i key={s.key} style={{ width: `${((s.pct ?? 0) / segTotal) * 100}%`, background: s.bg }}/>))}
            </span>)}
        </div>

        {deltaRounded != null && (<div style={{ width: 62, flex: '0 0 62px', textAlign: 'right' }}>
        ...
        {!hasScoring && (<span style={{ color: A.DIM, fontSize: 16, flexShrink: 0, lineHeight: 1.2 }}>{CHEVRON}</span>)}
      </div>
    </>);
    const shell = {
        display: 'block',
    ...
    if (!hasScoring) {
        return (<button type="button" onClick={() => onOpen('row')} className="active:scale-[0.99]" style={{ ...shell, cursor: 'pointer' }}>
        {body}
      </button>);
    }
    return (<div>
      ...
      {expanded && (<div style={{ padding: '0 20px 14px' }}>
          <div style={{
                display: 'grid',
            ...
            {segs.map((s) => (<div key={s.key} style={{ textAlign: 'center', minWidth: 0 }}>
          ...
          <Action label={t('yourCourses.openAnalytics')} onClick={() => onOpen('expanded')} style={{ marginTop: 10 }}/>
        </div>)}
    </div>);
}
export default function YourCourseAnalyticsSheet({ open, onClose, onNavigate, synced }) {
    const { t } = useTranslation('courses');
    ...
    const listItems = useMemo(() => myCourses, [myCourses]);
    /**
    ...
    const totalRounds = useMemo(() => listItems.reduce((s, c) => s + (c.rounds_count ?? 0), 0), [listItems]);
    useEffect(() => {
        if (!open)
    ...
    useEffect(() => {
        ...
        if (viewedRef.current || isLoading)
            return;
    ...
    const go = useCallback((courseId, from) => {
    ...
    const toggle = useCallback((course, delta) => {
        setExpandedIds((prev) => {
    ...
    const subLine = signedBaseline != null
    ...
    return (<BottomSheet open={open} onClose={onClose} ariaLabelledBy="your-course-analytics-title" zIndexBase={10000} maxHeight="75dvh" 
    ...
    style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{
        ...
        <div style={{ padding: '8px 20px 12px', flexShrink: 0 }}>
          ...
          <h2 id="your-course-analytics-title" style={{
            margin: '4px 0 0',
            fontSize: 17,
            fontWeight: 800,
            color: A.INK,
        ...
        {showSearchField && (<div style={{ padding: '0 20px 12px', flexShrink: 0 }}>
            <div style={{
                display: 'flex',
                alignItems: 'center',
                gap: 7,
              ...
              <input type="text" value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('yourCourses.searchPlaceholder')} style={{
                flex: 1,
                minWidth: 0,
                fontSize: 14,
                fontFamily: SANS,
        ...
        <div style={{
          ...
          {/* Building state */}
          {showBuildingState && (<div style={{ padding: '4px 20px 20px' }}>
              <div style={{
                background: '#fff',
          ...
          {searchActive ? (<div style={{
              ...
              {searching && searchResults.length === 0 ? (<div style={{ padding: 16, ...CAPTION }}>{t('yourCourses.searching')}</div>) : searchResults.length === 0 ? (<div style={{ padding: 16, ...CAPTION }}>{t('yourCourses.noResults')}</div>) : (searchResults.map((c) => (<Row key={c.id} title={c.name} subtitle={[c.region, c.country].filter(Boolean).join(` ${DOT} `) || undefined} onClick={() => go(c.id, 'row')}/>)))}
            </div>) : showList ? (<>
              <div style={{
                margin: '0 20px',
                background: '#fff',
                ...
                {listItems.map((c) => {
                const delta = baseline != null && c.avg_to_par !== null && c.avg_to_par !== undefined
                    ? c.avg_to_par - baseline
                    : null;
                return (<AnalyticsCourseRow key={c.course_id} course={c} delta={delta} expanded={expandedIds.has(c.course_id)} onToggle={() => toggle(c, delta)} onOpen={(from) => go(c.course_id, from)}/>);
            })}
              </div>
              {signedBaseline != null && (<div style={{ ...CAPTION, padding: '14px 24px 4px', textAlign: 'center' }}>
                  {t('yourCourses.footnote', { avg: signedBaseline, rounds: totalRounds })}
                </div>)}
            </>) : null}

          {/* Loading skeleton for first paint */}
          {isLoading && !showBuildingState && !showList && (<div style={{ padding: '0 20px' }}>
              <div style={{
                background: '#fff',
                border: `1px solid ${A.BORDER}`,
                borderRadius: 16,
