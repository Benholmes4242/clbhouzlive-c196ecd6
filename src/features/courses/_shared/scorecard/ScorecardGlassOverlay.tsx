    ...
    >
      <div
        ...
        style={{
          ...
          opacity: entered ? 1 : 0,
          transform: `translateY(calc(-50% + ${dragY}px)) scale(${entered ? 1 : 0.95})`,
          transition: dragY > 0
            ? 'none'
            : 'opacity 200ms ease, transform 260ms cubic-bezier(.32,.72,0,1)',
          willChange: 'transform, opacity',
          zIndex: Z.sheet,
        }}
      >
        <style>{`@media (prefers-reduced-motion: reduce) { [data-scorecard-glass-card="true"] { transform: translateY(calc(-50% + ${dragY}px)) !important; transition: opacity 200ms ease !important; } }`}</style>
        {children}
      </div>
    </div>,
    document.body,
