  ...
  return (
    <div className="fixed inset-0 bg-[#0D0F11] z-50 flex flex-col overflow-y-auto">
      ...
      <div className="flex items-center justify-center py-5">
        <span className="text-white text-[18px] font-bold tracking-tight">clbhouz</span>
      </div>

      {/* Post card */}
      <div className="flex-1 flex flex-col items-center px-4 pb-4">
        <div className="w-full max-w-[400px] rounded-2xl overflow-hidden" style={{ background: '#161A1E' }}>
          {/* Media thumbnail */}
          <div className="relative w-full aspect-[4/5] overflow-hidden" style={{ background: gradient }}>
            ...
            {/* Course tag */}
            {course && (
              <div className="absolute bottom-3 left-3 flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-white/90 text-[12px] font-medium" style={{ background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(8px)' }}>
          ...
          <div className="p-4">
            <div className="flex items-center gap-3 mb-3">
              {profile?.avatar_url ? (
                ...
                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white/60 text-[14px] font-semibold">
                  {displayName.charAt(0).toUpperCase()}
                </div>
              )}
              <div className="flex flex-col">
                <span className="text-white text-[15px] font-semibold leading-tight">{displayName}</span>
                {username && (
                  <span className="text-white/40 text-[13px] leading-tight">@{username}</span>
                )}
              </div>
            </div>
            {post.caption && (
              <p className="text-white/70 text-[14px] leading-[1.45] line-clamp-4">
                {post.caption}
              </p>
            )}
          </div>
      ...
      {/* CTAs */}
      <div className="px-4 pb-6 flex flex-col gap-3 w-full max-w-[400px] mx-auto">
        {/* Primary — open in app */}
        <a
      ...
      <div className="py-4 flex items-center justify-center">
        <span className="text-white/20 text-[12px]">clbhouz · golf's digital clubhouse</span>
      </div>
    </div>
  );
};
