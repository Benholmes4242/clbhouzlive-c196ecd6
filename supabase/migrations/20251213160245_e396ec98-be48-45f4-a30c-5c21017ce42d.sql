-- Update audit log entries to use clean, legal-safe copy

-- 1) Update verification submitted trigger to use audit copy
...
BEGIN
  ...
  -- Insert notification with standardized copy
  ...
  -- Log to audit with clean, neutral copy
  INSERT INTO public.verification_audit_log (
    ...
    'submitted',
    new.requested_by,
    'Verification request submitted by business owner.',
    jsonb_build_object(
      'request_id', new.id,
      'proof_method', new.proof_method
    )
  );
...
-- 2) Update status change trigger to use audit copy for approved
...
BEGIN
  ...
  IF new.status = 'approved' THEN
    ...
    _audit_reason := 'Business verification approved.';
  ELSIF new.status = 'rejected' THEN
    ...
    _audit_reason := 'Business verification rejected.';
  ELSE
    RETURN new;
  END IF;
...
-- 3) Update revoke function - audit already uses clean copy, just ensure consistency
...
BEGIN
  ...
  -- Log to audit with clean, neutral copy (admin reason stored separately in metadata)
