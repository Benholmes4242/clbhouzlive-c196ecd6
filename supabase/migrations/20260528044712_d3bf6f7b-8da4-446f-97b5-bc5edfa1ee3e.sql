    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'dist', jsonb_build_object(
          'birdie_better', round(100.0 * count(*) FILTER (WHERE wsh.actual_gross - wsh.par <= -1) / count(*), 1),
          'par',          round(100.0 * count(*) FILTER (WHERE wsh.actual_gross - wsh.par = 0)  / count(*), 1),
          'bogey',        round(100.0 * count(*) FILTER (WHERE wsh.actual_gross - wsh.par = 1)  / count(*), 1),
          'double_worse', round(100.0 * count(*) FILTER (WHERE wsh.actual_gross - wsh.par >= 2) / count(*), 1)
        )
      ) AS h
