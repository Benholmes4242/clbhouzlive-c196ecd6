...
BEGIN
  ...
  tournament_card AS (
    SELECT
      ...
      COALESCE(tp.actor_type, 'system')::text AS p_actor_type,
      tp.actor_id AS p_actor_id,
      tp.status AS p_status,
      NULL::uuid AS p_source_review_id,
      NULL::uuid AS pm_id,
      NULL::text AS pm_media_type,
      NULL::text AS pm_media_url,
      NULL::text AS pm_poster_url,
      NULL::text AS pm_stream_id,
      NULL::numeric AS pm_duration,
      NULL::int AS pm_width,
      NULL::int AS pm_height,
      NULL::int AS pm_display_order,
      0::bigint AS p_like_count,
      0::bigint AS p_comment_count,
      0::bigint AS p_share_count,
      'system'::text AS p_relation,
      FALSE AS p_liked_by_me,
      FALSE AS p_followed_by_me,
      NULL::numeric AS p_review_rating,
      NULL::uuid AS p_review_course_id,
      NULL::text AS p_review_course_name,
      NULL::text AS p_review_course_image,
      NULL::text AS p_review_course_region,
      NULL::text AS p_review_course_country,
      NULL::text AS p_review_course_sub_country,
      'clbhouz'::text AS p_creator_username,
      'Clbhouz'::text AS p_creator_display_name,
      NULL::text AS p_creator_avatar,
      TRUE AS p_creator_verified,
      NULL::text AS p_business_name,
      NULL::text AS p_business_logo,
      FALSE AS p_business_verified,
      tp.post_type AS p_post_type,
      row_to_json(trm.*)::jsonb AS p_tournament_meta,
      (CASE
        WHEN EXTRACT(EPOCH FROM (now() - tp.created_at))/3600 < 6
          THEN trm.tour_priority * 1.0
        WHEN EXTRACT(EPOCH FROM (now() - tp.created_at))/3600 < 24
          THEN trm.tour_priority * 0.5
        WHEN EXTRACT(EPOCH FROM (now() - tp.created_at))/3600 < 72
          THEN (COALESCE(tp.like_count,0) * 1.0 + COALESCE(tp.comment_count,0) * 2.5)
            * EXP(-0.693 * EXTRACT(EPOCH FROM (now() - tp.created_at))/3600/24)
        ELSE
          (COALESCE(tp.like_count,0) * 1.0 + COALESCE(tp.comment_count,0) * 2.5)
            * EXP(-0.693 * EXTRACT(EPOCH FROM (now() - tp.created_at))/3600/48)
      END)::numeric AS score,
      1::bigint AS creator_rank
    FROM posts tp
    JOIN tournament_result_meta trm ON trm.post_id = tp.id
    WHERE tp.post_type = 'tournament_result'
      AND tp.status = 'published'
      AND tp.created_at > now() - interval '5 days'
    ORDER BY
      CASE WHEN tp.id = ANY(p_seen_post_ids) THEN 1 ELSE 0 END ASC,
      trm.tour_priority DESC
    LIMIT 1
  ),
  combined AS (
    SELECT
      ...
      f.p_creator_username, f.p_creator_display_name, f.p_creator_avatar,
      f.p_creator_verified, f.p_business_name, f.p_business_logo, f.p_business_verified,
      f.p_post_type, f.p_tournament_meta, f.score
    FROM filtered f

    UNION ALL

    SELECT
      t.p_id, t.p_content, t.p_created_at, t.p_user_id, t.p_actor_type, t.p_actor_id,
      ...
      t.p_review_course_sub_country,
      t.p_creator_username, t.p_creator_display_name, t.p_creator_avatar,
      t.p_creator_verified, t.p_business_name, t.p_business_logo, t.p_business_verified,
      t.p_post_type, t.p_tournament_meta, t.score
  ...
  SELECT
    ...
    ranked.p_actor_id,
    ranked.p_status,
    ranked.p_source_review_id,
    ranked.pm_id,
    ranked.pm_media_type,
    ranked.pm_media_url,
    ranked.pm_poster_url,
    ranked.pm_stream_id,
    ranked.pm_duration,
    ranked.pm_width,
    ranked.pm_height,
    ranked.pm_display_order,
    ranked.p_creator_username,
    ranked.p_creator_display_name,
    ranked.p_creator_avatar,
    ranked.p_creator_verified,
    ranked.p_business_name,
    ranked.p_business_logo,
    ranked.p_business_verified,
    ranked.p_like_count,
    ranked.p_comment_count,
    ranked.p_share_count,
    ranked.p_review_rating,
    ranked.p_review_course_id,
    ranked.p_review_course_name,
    ranked.p_review_course_image,
    ranked.p_review_course_region,
    ranked.p_review_course_country,
    ranked.p_review_course_sub_country,
    ranked.p_relation,
    ranked.p_liked_by_me,
    ranked.p_followed_by_me,
    ranked.score,
    ranked.p_post_type,
    ranked.p_tournament_meta
  FROM (
    SELECT c.*,
      ROW_NUMBER() OVER (ORDER BY
        CASE WHEN c.p_post_type = 'tournament_result' THEN 1 ELSE 0 END ASC,
        c.score DESC
      ) AS rn,
