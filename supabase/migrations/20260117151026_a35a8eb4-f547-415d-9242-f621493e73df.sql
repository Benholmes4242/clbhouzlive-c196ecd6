...
-- Create function to get top video reviews for "Reviews of the Week" carousel
...
BEGIN
  ...
  SELECT 
    ...
    CONCAT(COALESCE(gc.sub_country, gc.city, ''), CASE WHEN gc.sub_country IS NOT NULL OR gc.city IS NOT NULL THEN ', ' ELSE '' END, gc.country) as course_location,
...
GRANT EXECUTE ON FUNCTION get_top_video_reviews(INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION get_top_video_reviews(INTEGER, INTEGER) TO anon;
