...
CREATE OR REPLACE FUNCTION public.get_friend_leaderboard(p_user_id uuid)
 RETURNS TABLE(
   ...
   friend_row_id uuid,
   friend_name text,
   friend_thumbnail_url text,
   friend_profile_photo_url text,
   friend_handicap_index numeric,
   friend_home_club text,
   last_round_played_at timestamp with time zone,
   last_round_course_name text,
   is_clbhouz_user boolean,
   handicap_30d_ago numeric,
   handicap_30d_delta numeric,
   rounds_last_30d integer
 )
 LANGUAGE sql
 STABLE
AS $function$
