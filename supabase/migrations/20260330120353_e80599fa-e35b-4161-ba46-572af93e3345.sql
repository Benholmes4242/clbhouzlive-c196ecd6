-- Add 'golf_club' to post_tags entity_type filter in both feed RPCs
...
BEGIN
  ...
  candidates AS (
    SELECT
      ...
      COALESCE(lc.cnt, 0) AS p_like_count, COALESCE(cc.cnt, 0) AS p_comment_count,
      ...
      up.profile_photo_url AS p_creator_avatar, COALESCE(up.is_verified, FALSE) AS p_creator_verified,
      ba.name AS p_business_name, ba.logo_url AS p_business_logo,
      COALESCE(ba.is_verified, FALSE) AS p_business_verified,
      p.post_type AS p_post_type, NULL::jsonb AS p_tournament_meta, cr.review AS p_review_text
    FROM posts p
    ...
    WHERE p.status = 'published' AND bu.uid IS NULL
  ...
  filtered AS (
    SELECT * FROM scored WHERE creator_rank <= v_creator_cap ORDER BY score DESC LIMIT p_page_size
  ),
  tournament_card AS (
    SELECT
      ...
      NULL::numeric AS pm_duration, NULL::int AS pm_width, NULL::int AS pm_height,
...
-- Recreate get_friends_feed with golf_club in post_tags filter
