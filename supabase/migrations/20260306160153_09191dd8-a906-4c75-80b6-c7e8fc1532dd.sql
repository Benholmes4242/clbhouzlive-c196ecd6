...
-- Create get_long_form_videos RPC for the Videos tab
-- Returns long-form video posts (duration >= 240s / 4 minutes)
-- Supports latest, popular, following modes with cursor pagination
...
DECLARE
  ...
  v_max_post_age_days INT := 30;
  v_min_duration_seconds INT := 240;
  v_creator_cap INT := 3;
...
BEGIN
  ...
  candidates AS (
    ...
    WHERE p.status = 'published'
      ...
      AND pm.duration_seconds >= v_min_duration_seconds
      AND (p_search_query IS NULL OR p_search_query = '' OR
        p.content ILIKE '%' || p_search_query || '%' OR
  ...
  filtered AS (
    ...
    ORDER BY
      CASE WHEN p_mode = 'popular' THEN score ELSE 0 END DESC,
      CASE WHEN p_mode IN ('latest', 'following') THEN p_created_at END DESC NULLS LAST
    LIMIT p_page_size
  )
  SELECT
    f.p_id,
  ...
  FROM filtered f
  ORDER BY
    CASE WHEN p_mode = 'popular' THEN f.score ELSE 0 END DESC,
    CASE WHEN p_mode IN ('latest', 'following') THEN f.p_created_at END DESC NULLS LAST;
END;
$$;
