-- Fix get_friends_feed with 3 changes:
-- 1. Remove DISTINCT ON to preserve multi-media posts
-- 2. Add 90-day post age filter
-- 3. Add creator cap of 5 in latest mode
...
BEGIN
  ...
  WHERE (
    (v_mode = 'popular' AND s.creator_rank <= 3)
    OR (v_mode = 'latest' AND s.creator_rank <= 5)
  )
  ORDER BY
