...
BEGIN
  ...
  -- Every value wrapped in to_jsonb(COALESCE(...)) so no operand can be SQL NULL
  -- (jsonb_build_object tolerates JSON nulls, but we belt-and-brace anyway).
...
EXCEPTION WHEN OTHERS THEN
  -- Log loudly so silent swallows show up in Postgres logs.
  RAISE WARNING 'auto_queue_push_notification failed notif_id=% type=% recipient_actor_type=% recipient_actor_id=% user_id=% actor_id=% sqlstate=% sqlerrm=%',
    NEW.id, NEW.type, NEW.recipient_actor_type, NEW.recipient_actor_id, NEW.user_id, NEW.actor_id, SQLSTATE, SQLERRM;
  RETURN NEW;
END;
$function$;