...
BEGIN
  ...
  my_friends AS (
    ...
    WHERE uf.friend_id = v_user_id AND uf.status = 'accepted'
  ),
  ...
  candidates AS (
    ...
    LEFT JOIN LATERAL (SELECT COUNT(*) AS cnt FROM post_likes pl WHERE pl.post_id = p.id) plc ON TRUE
    LEFT JOIN LATERAL (SELECT COUNT(*) AS cnt FROM post_comments pc WHERE pc.post_id = p.id) pcc ON TRUE
    LEFT JOIN LATERAL (SELECT COUNT(*) AS cnt FROM post_shares ps WHERE ps.post_id = p.id) psc ON TRUE
    ...
    LEFT JOIN post_likes ml ON ml.post_id = p.id AND ml.user_id = v_user_id
    ...
    WHERE p.status = 'published'
      ...
      -- Region filter: if region specified, post must be associated with a course in that region's countries
      AND (
        v_region IS NULL
        OR gc.country IN (SELECT country FROM region_countries)
      )
      -- Search filter
      AND (
        v_search_query IS NULL
        OR p.content ILIKE '%' || v_search_query || '%'
        OR up.username ILIKE '%' || v_search_query || '%'
        OR up.display_name ILIKE '%' || v_search_query || '%'
        OR gc.name ILIKE '%' || v_search_query || '%'
      )
    ORDER BY p.id, pm.id, p.created_at DESC
  ...
  scored AS (
    SELECT
      ...
      GREATEST(
        ...
        * CASE
            WHEN c.pm_media_type = 'video' THEN 3.0
            WHEN c.p_source_review_id IS NOT NULL THEN 2.0
            ELSE 1.0
          END,
        CASE
          WHEN c.pm_media_type = 'video' THEN 0.1
          WHEN c.p_source_review_id IS NOT NULL THEN 0.05
          ELSE 0.001
        END
      ) AS score,
      ROW_NUMBER() OVER (
