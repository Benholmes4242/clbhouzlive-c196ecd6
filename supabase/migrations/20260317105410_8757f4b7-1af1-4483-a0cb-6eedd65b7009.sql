...
-- Drop both functions first to allow return type change
DROP FUNCTION IF EXISTS public.get_suggested_feed(uuid, integer, timestamptz, uuid[]);
DROP FUNCTION IF EXISTS public.get_friends_feed(uuid, text, integer, timestamptz, uuid[], text);

-- Recreate get_suggested_feed with post_tags
...
BEGIN
    ...
    SELECT c.*,
      (
        ...
        * POWER(0.5, EXTRACT(EPOCH FROM (NOW() - c.p_created_at)) / 3600 / v_recency_halflife_hours)
        + CASE WHEN c.p_relation = 'friend' THEN 3 WHEN c.p_relation = 'following' THEN 1.5 ELSE 0 END
        + CASE WHEN c.p_source_review_id IS NOT NULL THEN 2 ELSE 0 END
      ) AS score,
      ROW_NUMBER() OVER (
  ...
  tournament_card AS (
    ...
    ORDER BY CASE WHEN tp.id = ANY(p_seen_post_ids) THEN 1 ELSE 0 END ASC, trm.tour_priority DESC
    LIMIT 1
  ),
  combined AS (
...
-- Recreate get_friends_feed with post_tags
CREATE OR REPLACE FUNCTION public.get_friends_feed(p_user_id uuid, p_mode text DEFAULT 'latest'::text, p_page_size integer DEFAULT 15, p_cursor timestamp with time zone DEFAULT NULL::timestamp with time zone, p_seen_post_ids uuid[] DEFAULT '{}'::uuid[], p_search_query text DEFAULT NULL::text)
 RETURNS TABLE(post_id uuid, post_content text, post_created_at timestamp with time zone, post_user_id uuid, post_actor_type text, post_actor_id uuid, post_status text, source_review_id uuid, media_id uuid, media_type text, media_url text, poster_url text, stream_id text, duration_seconds numeric, width integer, height integer, display_order integer, creator_username text, creator_display_name text, creator_avatar_url text, creator_is_verified boolean, business_name text, business_logo_url text, business_is_verified boolean, like_count bigint, comment_count bigint, share_count bigint, review_rating numeric, review_course_id uuid, review_course_name text, review_course_image text, review_course_region text, review_course_country text, review_course_sub_country text, course_region text, course_country text, creator_relation text, is_liked_by_me boolean, is_followed_by_me boolean, engagement_score numeric, review_text text, post_tags jsonb)
 LANGUAGE plpgsql
...
BEGIN
  ...
  WITH social_graph AS (
    ...
    WHERE uf.user_id = v_user_id AND uf.status = 'accepted'
    UNION
    SELECT uf2.user_id AS target_user_id, 'personal'::TEXT, 'friend'::TEXT
    FROM user_friends uf2
    WHERE uf2.friend_id = v_user_id AND uf2.status = 'accepted'
    UNION
    SELECT ufl.following_id AS target_user_id, 'personal'::TEXT, 'following'::TEXT
    FROM user_follows ufl
    WHERE ufl.follower_id = v_user_id
    UNION
    SELECT bf.business_id AS target_user_id, 'business'::TEXT, 'following'::TEXT
    FROM business_follows bf
    WHERE bf.follower_id = v_user_id
  ),
  blocked_users AS (
  ...
  raw_posts AS (
    SELECT DISTINCT ON (p.id)
      ...
      pm.poster_url AS pm_poster_url,
      pm.stream_id AS pm_stream_id,
      pm.duration_seconds::numeric AS pm_duration,
      pm.width AS pm_width,
      pm.height AS pm_height,
      pm.display_order AS pm_display_order,
      sg.rel AS sg_rel,
      sg.target_type AS sg_target_type
    FROM posts p
    INNER JOIN post_media pm ON pm.post_id = p.id
    INNER JOIN social_graph sg ON (
      (sg.target_type = 'personal' AND p.user_id = sg.target_user_id)
      OR
      (sg.target_type = 'business' AND p.actor_type = 'business' AND p.actor_id = sg.target_user_id)
    )
    WHERE p.status = 'published'
      ...
      AND (
        ...
        OR lower(COALESCE(
          (SELECT up.display_name FROM user_profiles up WHERE up.id = p.user_id),
          ''
        )) LIKE v_search_pattern
      )
  ...
  SELECT
    s.id AS post_id,
    s.content AS post_content,
    s.created_at AS post_created_at,
    s.user_id AS post_user_id,
    COALESCE(s.actor_type, 'personal') AS post_actor_type,
    s.actor_id AS post_actor_id,
    s.status AS post_status,
    s.source_review_id,
    s.pm_id AS media_id,
    s.pm_media_type AS media_type,
    s.pm_media_url AS media_url,
    s.pm_poster_url AS poster_url,
    s.pm_stream_id AS stream_id,
    s.pm_duration AS duration_seconds,
    s.pm_width AS width,
    s.pm_height AS height,
