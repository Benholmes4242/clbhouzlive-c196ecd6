...
DECLARE
  v_season_id uuid;
BEGIN
  -- Determine the target season
  IF target_season_id IS NOT NULL THEN
    v_season_id := target_season_id;
  ELSE
    SELECT id INTO v_season_id
    FROM sr_seasons
    WHERE tour_id IN (SELECT id FROM sr_tours WHERE slug = 'pga')
    ORDER BY year DESC
    LIMIT 1;
  END IF;
...
-- Also update the auto-refresh wrapper to use the same logic
...
BEGIN
  ...
  JOIN sr_tours t ON t.id = s.tour_id AND t.slug = 'pga'
  WHERE ps.earnings > 0
