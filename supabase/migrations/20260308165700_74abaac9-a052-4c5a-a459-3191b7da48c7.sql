...
BEGIN
  ...
  SELECT
    ...
    -- Engagement counts
    COALESCE(plc.cnt, 0)                   AS like_count,
    COALESCE(pcc.cnt, 0)                   AS comment_count,
  ...
  -- Engagement lateral subqueries
  LEFT JOIN LATERAL (
    SELECT COUNT(*)::BIGINT AS cnt FROM post_likes pl WHERE pl.post_id = p.id
  ) plc ON TRUE
  LEFT JOIN LATERAL (
    SELECT COUNT(*)::BIGINT AS cnt FROM post_comments pc WHERE pc.post_id = p.id
  ) pcc ON TRUE
  LEFT JOIN LATERAL (
    SELECT COUNT(*)::BIGINT AS cnt FROM post_shares ps WHERE ps.post_id = p.id
  ) psc ON TRUE
  WHERE p.status = 'published'
    -- Actor filter: for personal, include posts by user_id where actor_type is personal or null
    AND (
