...
BEGIN
  ...
  /* Course identity fields — always fetched so route-driven pages can render
     header/title even when there's no WHS mapping. */
  ...
  /* Resolve the WHS course_id via the mapping table.
     Only trust mappings that are EITHER reviewed OR have confidence >= 0.70. */
  ...
  /* Build friend list (self + accepted friends) */
  ...
  /* Your rounds + best — uses adjusted_gross (actual_gross isn't synced) */
  ...
  /* Course par + CR + slope (most recent round with full hole data) */
  SELECT
    (SELECT SUM(par) FROM public.whs_score_holes WHERE score_id = ws.id),
    ws.course_rating,
    ws.slope_rating
  INTO v_course_par, v_cr, v_slope
  FROM public.whs_scores ws
  WHERE ws.course_id = v_whs_course_id
  ...
  /* Course difficulty — uses adjusted_gross */
  ...
  /* Hardest hole (most recent round with SI 1 data) */
