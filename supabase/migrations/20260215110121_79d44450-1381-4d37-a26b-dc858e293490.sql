...
-- Create a reusable function to refresh college weekly movers
-- It snapshots current college_season_stats and computes week-over-week deltas
...
BEGIN
  ...
  ON CONFLICT (season_id, normalized_name, week_start) DO UPDATE SET
    ...
    cuts_total = EXCLUDED.cuts_total,
    top10_total = EXCLUDED.top10_total,
    top25_total = EXCLUDED.top25_total,
    player_count = EXCLUDED.player_count,
    events_total = EXCLUDED.events_total;
  
  -- Compute movers by comparing this week's snapshot to last week's
