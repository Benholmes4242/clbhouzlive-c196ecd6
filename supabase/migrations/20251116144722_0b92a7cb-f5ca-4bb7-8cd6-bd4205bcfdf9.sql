-- Fixed backfill using correct country column
...
BEGIN
  -- Clear existing data
  DELETE FROM course_top100_memberships;
  ...
  LOOP
    ...
    ELSIF list_rec.slug = 'europe' THEN
      WITH ranked_courses AS (
        ...
        WHERE regional_rank IS NOT NULL 
          AND regional_rank > 0
          AND country = 'Continental Europe'
      )
      INSERT INTO course_top100_memberships (course_id, list_id, rank)
      SELECT course_id, list_rec.id, regional_rank
      FROM ranked_courses
    ...
    RETURN QUERY SELECT 
      ...
      list_rec.slug,
      added_count,
      format('%s courses added', added_count);
  END LOOP;
END;
$$;
