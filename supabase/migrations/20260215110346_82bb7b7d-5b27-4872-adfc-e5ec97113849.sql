...
-- Fix: Select season based on what college_season_stats actually uses
...
DECLARE
  ...
  last_monday DATE;
BEGIN
  -- Get the season that college_season_stats actually uses
  SELECT DISTINCT season_id INTO current_season_id 
  FROM public.college_season_stats 
  LIMIT 1;
  
  IF current_season_id IS NULL THEN
    RAISE NOTICE 'No college season stats found, skipping refresh';
    RETURN;
  END IF;
