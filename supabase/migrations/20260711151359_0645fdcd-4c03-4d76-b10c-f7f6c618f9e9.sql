...
-- 1) Suggested feed: exclude blocked authors (both directions)
...
BEGIN
  ...
  WITH
  blocked_users AS (
    SELECT blocked_id AS uid FROM public.user_blocks WHERE blocker_id = p_user_id
    UNION
    SELECT blocker_id AS uid FROM public.user_blocks WHERE blocked_id = p_user_id
  ),
  my_following AS (
...
-- 2) Inbox: exclude conversations that include a personal participant the
--    viewer has blocked (or who has blocked the viewer). Only applied when
--    the viewer's own actor is personal — business accounts unaffected.
CREATE OR REPLACE FUNCTION public.get_inbox(p_actor_type text, p_actor_id uuid)
RETURNS TABLE(conversation_id uuid, type text, title text, avatar_url text, last_message_at timestamp with time zone, last_message_preview text, unread_count bigint, is_muted boolean, is_archived boolean, my_role text, participants jsonb)
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  IF NOT msg_can_act_as(p_actor_type, p_actor_id) THEN
  ...
  SELECT
    ...
    (SELECT count(*) FROM messages m
       LEFT JOIN messages lr ON lr.id = me.last_read_message_id
       WHERE m.conversation_id = c.id AND m.deleted_at IS NULL
         AND NOT (m.sender_actor_type = me.actor_type AND m.sender_actor_id = me.actor_id)
         AND (
    ...
    (me.muted_until IS NOT NULL AND me.muted_until > now()) AS is_muted,
    (me.archived_at IS NOT NULL) AS is_archived,
    me.role AS my_role,
    (SELECT jsonb_agg(jsonb_build_object(
        'actor_type', p.actor_type, 'actor_id', p.actor_id, 'role', p.role,
        'name', prof.pname, 'avatar_url', prof.pavatar,
        'username', prof.puser, 'verified', prof.pverified))
     FROM conversation_members p
     LEFT JOIN LATERAL (
       SELECT up.display_name AS pname, up.profile_photo_url AS pavatar,
              up.username AS puser, up.is_verified AS pverified
       FROM user_profiles up WHERE p.actor_type='personal' AND up.id = p.actor_id
       UNION ALL
       SELECT ba.name, ba.logo_url, ba.slug, ba.is_verified
       FROM business_accounts ba WHERE p.actor_type='business' AND ba.id = p.actor_id
     ) prof ON true
     WHERE p.conversation_id = c.id AND p.left_at IS NULL
    ) AS participants
  FROM conversation_members me
  JOIN conversations c ON c.id = me.conversation_id
  WHERE me.actor_type = p_actor_type AND me.actor_id = p_actor_id AND me.left_at IS NULL
    AND NOT (
      ...
      AND EXISTS (
        ...
        JOIN public.user_blocks ub ON (
          (ub.blocker_id = p_actor_id AND ub.blocked_id = other.actor_id)
          OR (ub.blocked_id = p_actor_id AND ub.blocker_id = other.actor_id)
        )
        WHERE other.conversation_id = c.id
          AND other.actor_type = 'personal'
          AND NOT (other.actor_type = p_actor_type AND other.actor_id = p_actor_id)
      )
    )
