...
RETURNS TABLE(
  ...
  duration_seconds INT, width INT, height INT, display_order INT,
...
BEGIN
  ...
  -- Posts-based media (existing logic)
  posts_media AS (
    SELECT
  ...
  -- Collect media_urls already covered by posts so we can deduplicate
  posts_media_urls AS (
    SELECT pm_media_url FROM posts_media
  ),
  -- Review-only media: items in course_review_media not already in post_media
  ...
  SELECT * FROM (
    ...
    SELECT
      -- Use review_id as a synthetic post_id so frontend can group/display
