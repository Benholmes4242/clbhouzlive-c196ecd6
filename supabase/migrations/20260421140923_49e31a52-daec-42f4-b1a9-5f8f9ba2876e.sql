...
BEGIN
  ...
  WITH candidates AS (
    ...
    WHERE p.visibility = 'anyone'
      ...
      AND (
        -- Reviews always pass (any AR, any duration, image-only allowed)
        p.source_review_id IS NOT NULL
        OR
        -- Non-reviews: must have at least one renderable media item
        -- (vertical-ish video ≥ 4s OR portrait/square image)
        EXISTS (
          SELECT 1
          FROM post_media pm
          WHERE pm.post_id = p.id
            AND (
              -- Video: portrait/square AR (width/height ≤ 1.0) AND duration ≥ 4s
              (
                pm.media_type = 'video'
              ...
              OR
              -- Image: portrait or square (width ≤ height), or unknown dimensions
              (
                pm.media_type = 'image'
                AND (
                  pm.width IS NULL
