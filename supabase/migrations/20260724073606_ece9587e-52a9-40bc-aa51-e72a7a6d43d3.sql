...
BEGIN
  ...
  -- Same filters as get_my_hole_performance: played, non-null gross, non-null par.
  holes_played AS (
    SELECT
      mapped.gc_id,
  ...
  -- Personal scoring distribution: user's own hole outcomes at this course.
  -- NOT the community distribution (that lives in get_course_hole_analysis).
  bucket_counts AS (
    SELECT
      gc_id,
      COUNT(*)                            AS total,
  ...
  dist_fixed AS (
    -- Largest-remainder adjustment on the bucket with the biggest raw count,
    -- so the four whole-number percentages always sum to exactly 100.
    SELECT
      gc_id,
