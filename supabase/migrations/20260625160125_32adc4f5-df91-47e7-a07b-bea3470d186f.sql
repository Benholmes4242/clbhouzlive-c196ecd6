...
BEGIN
  ...
  primary_media AS (
    ...
    WHERE pm.processing_status = 'complete'
      AND (
        (v_source = 'played'       AND pm.derived_format IN ('clip','video')) OR
        (v_source = 'want_to_play' AND pm.derived_format IN ('clip','video','image'))
      )
  ),
  course_posts AS (
    SELECT
    ...
    WHERE (v_source <> 'played' OR p.created_at > now() - interval '30 days')
  ...
  agg AS (
    ...
    HAVING COUNT(*) >= CASE WHEN v_source = 'want_to_play' THEN 1 ELSE 2 END
  )
  SELECT
