...
BEGIN
  ...
  IF p_mood = 'foryou' THEN
    ...
    JOIN course_mood_blurbs mb ON mb.course_id = gc.id AND mb.mood = 'foryou'
       AND (mb.user_id = p_user_id OR mb.user_id IS NULL) AND mb.expires_at > now()
       AND mb.blurb IS NOT NULL
  ...
  ELSIF p_mood = 'weekend' THEN
    RETURN QUERY
    SELECT gc.id, gc.name, gc.country, COALESCE(gc.region, gc.sub_country),
           gc.thumbnail_image, cra.avg_overall_score, cra.review_count::int, gc.global_rank,
           mb.blurb, jsonb_build_object('fallback', true), 'Saturday AM'::text
    FROM golf_courses gc
    JOIN course_rating_aggregates cra ON cra.course_id = gc.id
    ...
    WHERE gc.id <> ALL(v_played_ids)
      AND (v_hero_id IS NULL OR gc.id <> v_hero_id)
      AND (gc.country ILIKE '%united kingdom%' OR gc.country ILIKE '%britain%'
           OR gc.sub_country IN ('England','Scotland','Wales','Northern Ireland','Ireland'))
    ORDER BY cra.avg_overall_score DESC NULLS LAST, cra.review_count DESC
    LIMIT p_limit;
  ...
  ELSIF p_mood = 'friends' THEN
    ...
    SELECT fr.course_id, fr.course_name, gc.country, fr.region, fr.thumbnail_image,
           fr.rating_avg, fr.review_count, gc.global_rank, mb.blurb,
           jsonb_build_object('friends_played_count', fr.friend_played_count,
