-- Fix Friends First RPC: Don't exclude user's own posts, just don't prioritize them as "friends"
-- Also fix ordering to exclude self from friend bucket
...
AS $$
  ...
  SELECT 
    fp.id AS post_id,
    -- is_friend = followed user AND not self
  ...
  ORDER BY
    -- Friends first (excluding self from friend bucket)
