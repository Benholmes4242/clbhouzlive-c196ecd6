-- Revert notifications.actor_id to personal user (FK to users); keep business identity in data only.
...
BEGIN
  ...
  ELSIF post_record.actor_type = 'business' THEN
    ...
    FOR business_team_id IN
      ...
      WHERE business_id = post_record.actor_id AND role IN ('owner', 'admin')
    LOOP
      IF business_team_id = liker_user_id AND liker_actor_type = 'personal' THEN CONTINUE; END IF;
      IF are_users_blocked(liker_user_id, business_team_id) THEN CONTINUE; END IF;
      ...
      ELSE
        ...
        VALUES (
          business_team_id, recipient_actor_id_val, recipient_actor_type_val, liker_user_id,
...
-- Legacy single-like function (unused by triggers but kept actor-aware for safety) — actor_id stays personal.
...
BEGIN
  ...
  PERFORM public.send_push_notification(
    post_owner_id, 'like', 'Post Liked',
    CASE
      WHEN course_name IS NOT NULL THEN liker_name || ' liked your Moment at ' || course_name || '.'
      ELSE liker_name || ' liked your post.'
    END,
    jsonb_build_object(
      'post_id', NEW.post_id,
