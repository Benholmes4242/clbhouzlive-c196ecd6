-- Single-user RPC with viewer-aware visibility
...
SELECT jsonb_build_object(
  ...
  'additional_count', (SELECT (additional->>'count')::int FROM additional_payload),
  'additional_preview', (SELECT additional->'preview' FROM additional_payload)
);
$$;

GRANT EXECUTE ON FUNCTION public.get_home_clubs_for_user(uuid, uuid) TO authenticated;

-- Batch RPC for People grid
