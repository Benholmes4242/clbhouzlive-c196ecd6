-- ───────────────────────────────────────────────────────────────────
-- Watch tab: quality-floor scoring (Session 2 of 3)
--
-- Adds a duration_quality_score factor to the trending / top score
-- inside get_watch_shorts. The factor is multiplicative and small
-- (0.2 .. 1.0) so it acts as a *deprioritisation* of low-signal
-- content rather than a hard filter.
--
-- Buckets:
--   8s..180s   → 1.00  (sweet spot)
--   4s..7s     → 0.50  (short clips)
--   181s..300s → 0.50  (long clips, still under the v_max_duration)
--   < 4s       → 0.20  (very short)
--   > 300s     → 0.20  (effectively dead, but v_max_duration already caps at 180)
--
-- "latest" and "near" modes intentionally stay chronological — quality
-- scoring there would override the user's explicit ordering choice.
-- ───────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  scored AS (
    SELECT c.*,
      -- Duration quality factor — sweet spot 8..180s, gently penalises short and long clips.
      CASE
  ...
  scored_final AS (
    -- Multiply quality into trending/top only; latest & near stay strictly chronological
    -- so the user's explicit ordering choice is respected.
    SELECT s.*,
      CASE p_mode
        WHEN 'trending' THEN s.base_score * s.duration_quality
        WHEN 'top'      THEN s.base_score * s.duration_quality
        ELSE s.base_score
      END AS score
    FROM scored s
  ),
