-- Extend get_top100_leaderboard to include is_friend flag
...
BEGIN
  ...
  -- Build leaderboard with ranking and friend status
  ...
  leaderboard_data AS (
    SELECT 
      ...
      -- is_friend: true if this user is in the current user's friends list
      CASE 
        WHEN current_user_id IS NULL THEN false
        WHEN uc.user_id = current_user_id THEN false -- not a friend of yourself
        ELSE EXISTS (SELECT 1 FROM my_friends mf WHERE mf.friend_user_id = uc.user_id)
      END as is_friend
    FROM user_counts uc
