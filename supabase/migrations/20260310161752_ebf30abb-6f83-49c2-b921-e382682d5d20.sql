-- Drop and recreate get_suggested_feed with tournament_meta column and tournament UNION
...
BEGIN
  ...
  -- Tournament result posts: max 1 per RPC call, 5-day cutoff
  tournament_card AS (
    SELECT
      tp.id AS p_id,
      ...
      -- Score: tour_priority based with time decay
      CASE
  ...
  -- Combine regular feed with tournament card
  combined AS (
    SELECT
