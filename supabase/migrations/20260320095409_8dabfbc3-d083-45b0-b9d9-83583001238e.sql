...
BEGIN
  ...
  enriched AS (
    ...
    LEFT JOIN LATERAL (SELECT COUNT(*) AS cnt FROM post_comments pc WHERE pc.post_id = dp.p_id AND pc.deleted_at IS NULL) pcc ON TRUE
    ...
    LEFT JOIN LATERAL (SELECT pl2.user_id FROM post_likes pl2 WHERE pl2.post_id = dp.p_id AND pl2.user_id = v_user_id LIMIT 1) ml ON TRUE
