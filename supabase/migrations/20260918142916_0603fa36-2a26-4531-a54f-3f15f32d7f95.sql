...
BEGIN
  ...
  ELSIF NEW.target_type = 'round' THEN
    -- G7.1(d) — the round's owner. whs_scores has NO user_id: the owner is
    -- reached by connection_id -> whs_connections.user_id, and every reader in
    -- the app filters deleted_at IS NULL, so this branch makes that hop itself.
    -- HISTORY, AND A KNOWN DIVERGENCE: this read used to be
    --   v_recipient_uid := public.reaction_target_owner('round', NEW.target_id);
    -- "resolved by the same function the reaction trigger uses, so a like and a
    -- comment on a round tell the same person." That function does NOT filter
    -- whs_connections.deleted_at and is shared with the reaction trigger, so it
    -- is left untouched until its callers are enumerated. Until then a LIKE and
    -- a COMMENT on the same round resolve the owner by two rules that should
    -- never disagree in practice, because a round on a deleted connection is not
    -- visible to like or comment on at all. If the filter moves inside
    -- reaction_target_owner, both branches go back to calling it.
    -- Where the connection is missing, deleted, or carries no user, NO
    -- notification is raised -- no owner is guessed.
