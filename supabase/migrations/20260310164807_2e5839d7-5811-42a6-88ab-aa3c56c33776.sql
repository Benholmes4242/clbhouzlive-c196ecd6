...
BEGIN
  ...
  blocked_users AS (
    SELECT blocked_id AS uid FROM user_blocks
    WHERE blocker_id = p_user_id
    UNION
    SELECT blocker_id AS uid FROM user_blocks
    ...
    SELECT
      ...
      CASE
        WHEN EXTRACT(EPOCH FROM (now() - tp.created_at))/3600 < 6
          THEN trm.tour_priority::float * 1.0
        WHEN EXTRACT(EPOCH FROM (now() - tp.created_at))/3600 < 24
          THEN trm.tour_priority::float * 0.5
        WHEN EXTRACT(EPOCH FROM (now() - tp.created_at))/3600 < 72
          THEN (COALESCE(tp.like_count,0) * 1.0 + COALESCE(tp.comment_count,0) * 2.5 + COALESCE(tp.share_count,0) * 3.0)
            * EXP(-0.693 * EXTRACT(EPOCH FROM (now() - tp.created_at))/3600/24)
        ELSE
          (COALESCE(tp.like_count,0) * 1.0 + COALESCE(tp.comment_count,0) * 2.5 + COALESCE(tp.share_count,0) * 3.0)
  ...
  SELECT
    ...
    c.pm_height,
    c.pm_display_order,
    c.p_creator_username,
    c.p_creator_display_name,
    c.p_creator_avatar,
    c.p_creator_verified,
    c.p_business_name,
    c.p_business_logo,
    c.p_business_verified,
    c.p_like_count,
    c.p_comment_count,
    c.p_share_count,
    c.p_review_rating,
    c.p_review_course_id,
    c.p_review_course_name,
    c.p_review_course_image,
    c.p_review_course_region,
    c.p_review_course_country,
    c.p_review_course_sub_country,
    c.p_relation,
    c.p_liked_by_me,
    c.p_followed_by_me,
    c.score,
