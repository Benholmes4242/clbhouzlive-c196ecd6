-- Get Top 100 friends snapshot RPC
...
begin
  ...
  top100_counts as (
    select
      ap.user_id,
      coalesce(sum(uca.courses_played), 0)::integer as total_top100_played
    from all_players ap
...
-- Get user's Top 100 course IDs
create or replace function get_user_top100_course_ids(target_user_id uuid)
returns uuid[]
language sql
stable
as $$
  select coalesce(array_agg(distinct course_id), array[]::uuid[])
