...
serve(async (req) => {
  ...
  try {
    ...
    const { error: rpcErr } = await supabaseUser.rpc("approve_course_claim", {
      _request_id: request_id,
    });
    if (rpcErr) {
      console.error("approve_course_claim RPC failed:", rpcErr);
      throw new Error(rpcErr.message || "Failed to approve claim");
    }

    console.log(`Course claim ${request_id} approved by admin ${adminUserId}`);

    supabaseAdmin.functions
      .invoke("send-course-claim-result-email", {
        body: { business_id: request.business_id, outcome: "approved", admin_note: null },
      })
      .catch((e) => console.error("[approve-course-claim] result-email failed", e));

    return new Response(JSON.stringify({ ok: true }), {
      status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e: any) {
    console.error("approve-course-claim error:", e);
