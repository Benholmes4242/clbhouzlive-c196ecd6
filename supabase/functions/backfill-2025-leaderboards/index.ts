...
Deno.serve(async (req) => {
  ...
  const tourFilter: string | null = body.tour ?? null; // optional: 'pga','euro',...
  const limit: number = body.limit ?? 500;
  const delayMs: number = body.delayMs ?? 1300;
  const dryRun: boolean = !!body.dryRun;

  // Find candidate tournaments: closed, target year, zero leaderboard rows
  ...
  // Count existing leaderboard rows per tournament
