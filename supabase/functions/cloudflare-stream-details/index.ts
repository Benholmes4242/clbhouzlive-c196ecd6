...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiToken) {
      return new Response(
        JSON.stringify({ success: false, error: 'Cloudflare Stream API token not configured' }),
    ...
    if (!accountId) {
      return new Response(
        JSON.stringify({ success: false, error: 'Cloudflare Account ID not configured' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (!cloudflareResponse.ok) {
      return new Response(
        ...
        { status: cloudflareResponse.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
