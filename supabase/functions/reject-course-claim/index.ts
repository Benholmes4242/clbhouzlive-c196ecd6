...
serve(async (req) => {
  ...
  try {
    ...
    const { error: rpcErr } = await supabaseUser.rpc("reject_course_claim", {
      _request_id: request_id,
      _admin_note: admin_notes,
    });
    if (rpcErr) {
      console.error("reject_course_claim RPC failed:", rpcErr);
      throw new Error(rpcErr.message || "Failed to reject claim");
    }

    console.log(`Course claim ${request_id} rejected by admin ${adminUserId}`);

    supabaseAdmin.functions
      .invoke("send-course-claim-result-email", {
        body: { business_id: request.business_id, outcome: "rejected", admin_note: admin_notes },
      })
      .catch((e) => console.error("[reject-course-claim] result-email failed", e));

    return new Response(JSON.stringify({ ok: true }), {
      status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e: any) {
    console.error("reject-course-claim error:", e);
