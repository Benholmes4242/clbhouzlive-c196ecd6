-- Add creator handicap + home club fields to get_suggested_feed return,
-- so the Suggested tab's post overlay can render HCP and home course
-- (already wired in src/components/media-system/utils/feedMapper.ts:130-139,
-- and get_friends_feed already returns these columns).
--
-- Run in Supabase SQL editor. Drop required because RETURNS TABLE signature changes.

DROP FUNCTION IF EXISTS public.get_suggested_feed(
  uuid, integer, text, uuid[], text, numeric, numeric, numeric, numeric,
...
RETURNS TABLE(
  ...
  post_user_id uuid, post_actor_type text, post_actor_id uuid, post_status text,
  source_review_id uuid, media_id uuid, media_type text, media_url text,
  poster_url text, stream_id text, duration_seconds numeric, width integer,
  height integer, display_order integer, creator_username text,
  creator_display_name text, creator_avatar_url text, creator_is_verified boolean,
  business_name text, business_logo_url text, business_is_verified boolean,
  like_count bigint, comment_count bigint, share_count bigint, review_rating numeric,
  ...
  engagement_score numeric, post_type text, tournament_meta jsonb, review_text text,
  post_tags jsonb, course_id uuid, course_name text,
  review_design_score numeric, review_condition_score numeric,
  review_facilities_score numeric, review_clubhouse_score numeric,
  mutual_friends_count integer, country_match boolean,
  top100_list_match boolean, rated_post_course boolean,
  -- NEW: creator identity for post overlay parity with Friends tab
...
BEGIN
  ...
  with_media AS (
    SELECT
      ...
      -- NEW: creator identity (matches feedMapper.ts:130-139 expectations)
      up.eg_handicap_index AS wm_creator_handicap_index,
      up.show_handicap AS wm_creator_show_handicap,
      up.home_club AS wm_creator_home_club,
      up.home_club_visibility AS wm_creator_home_club_visibility
