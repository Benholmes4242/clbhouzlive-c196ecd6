-- =====================================================================
-- PHASE D HOTFIX (D5) - get_explore_stream: 42703 column g.id does not exist
--
-- THE DEPLOYED BODY IS BYTE-IDENTICAL TO docs/sql/explore_stream_d3.sql
-- (prosrc md5 2269e61522a052c8bae115621a5ff276, 32312 bytes). This file IS
-- that body with exactly TWO characters-level changes, nothing else:
--
--   c_low tiebreak : ORDER BY ... g.id ASC        -> g.whs_score_id ASC
--   rounds cid     : 'round:' || g.id::text       -> 'round:' || g.whs_score_id::text
--
-- WHY: public.gam_round_stats HAS NO id COLUMN in production. Its key is
-- whs_score_id (attnum 1). Both references were written against the local
-- harness fixture, which invented `id uuid primary key`. Because both live in
-- the ONE statement that every view shares, EVERY view - all, scores, courses
-- and reviews - errored 42703 on every call, and the D4 client fallback hid it.
--
-- cid is an opaque render/dedupe key; whs_score_id is already returned as its
-- own column, is NOT NULL for every candidate round (the rounds CTE requires
-- it) and is unique per round, so card identity and the keyset cursor are
-- unchanged in shape. Nothing else in the body is touched.
-- =====================================================================
-- ============================================================================
