-- ============================================================================
-- BRIEF_EXPLORE_MAGAZINE - PHASE D4 DRAFT. BEN RUNS THIS. NOT A MIGRATION.
-- ============================================================================
-- COMMENT-ONLY. The body below is the DEPLOYED D3 body, unchanged in every
-- executable line. The only change is this header, which writes down the two
-- behaviours the D3 harness ASSERTED but never EXPLAINED, so the next reader does
-- not mistake either one for a bug and 'fix' it.
--
-- 1. WHY A SHORT PAGE CAN STILL HAVE MORE.
--    The page limit is not a pool limit. The backlog allowance (default 1 row in
--    3) TURNS AWAY otherwise eligible historic rounds once a page has taken its
--    share of them; the cadence rule turns away a second consecutive card of the
--    same kind the same way. Those rows are not consumed - they stay in the pool
--    behind the cursor and are the FIRST candidates on the next page. So a page
--    of 8 where the limit is 10 means "this page had nothing else it was allowed
--    to place", NOT "the pool is spent". Emptiness is proved by the cursor coming
--    back with no rows at all, which is what the client's sentinel keys on.
--
-- 2. WHY A PAGE CAN COME BACK ONE CARD OVER THE LIMIT.
--    A card deferred by cadence or by the outer-ring cap is placed AFTER the
--    limit check for the slot that deferred it - the deferral is resolved at the
--    end of the assembly, not in the loop that counts. The overshoot is therefore
--    bounded at exactly ONE row per page and is never a second page's worth. It
--    is left as is deliberately: clamping it would drop a card the ranker had
--    already earned a place for, and the cursor is keyed on the LAST ROW ACTUALLY
--    RETURNED, so an over-long page cannot skip or duplicate anything.
-- ============================================================================
